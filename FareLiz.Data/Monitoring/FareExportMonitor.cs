namespace SkyDean.FareLiz.Data.Monitoring
{
    ...
    {
        /// <summary>The _archive manager.</summary>
        private readonly IArchiveManager _archiveManager;

        /// <summary>The _fare database.</summary>
        private readonly IFareDatabase _fareDatabase;

        /// <summary>The _data path.</summary>
        private readonly string _dataPath;

        /// <summary>The _cached routes.</summary>
        private readonly List<TravelRoute> _cachedRoutes = new List<TravelRoute>();

        /// <summary>The _auto sync.</summary>
        ...
        /// <summary>The _lock stream.</summary>
        private Stream _lockStream;

        /// <summary>The cach e_ amount.</summary>
        private const int CACHE_AMOUNT = 100;

#if DEBUG

        /// <summary>The loc k_ ext.</summary>
        ...
        /// <summary>The save data to database.</summary>
        private void SaveDataToDatabase()
        {
            IList<TravelRoute> newRoutes = null;
            lock (this._lockObj)
            {
                this._logger.Info("Trying to save data...");
                if (!Directory.Exists(this._dataPath))
                {
                    this._logger.Debug("Data path [{0}] no longer exists... Exiting...", this._dataPath);
                    return;
                }
                ...
                AppContext.ExecuteTask(
                    ...
                    this.GetType() + "SaveDataDb", 
                    ProgressStyle.Blocks, 
                    this._logger, 
                    callback => { newRoutes = this._archiveManager.ImportData(this._dataPath, DataOptions.Default, callback); });
            }
            ...
            if (this._autoSync() && syncDb != null)
            {
                AppContext.ExecuteTask(
                    ...
                    this.GetType() + "-SendData", 
                    ProgressStyle.Marquee, 
                    this._logger, 
                    callback =>
                        {
                            var newPkgId = syncDb.SendData(newRoutes, callback);
                            syncDb.AddPackage(newPkgId, null, callback);
                        });
            }
        }
        ...
        private void FareExportMonitor_OnRequestCompleted(FareRequestMonitor sender, FareBrowserRequestArg args)
        {
            ...
            lock (this._lockObj)
            {
                ...
                this._logger.Debug("There are {0} pending requests, {1} processing requests", pendingCount, processCount);
                var isLastRequest = pendingCount == 0 && processCount == 0;
        ...
        /// <summary>The flush cache.</summary>
        private void FlushCache()
        {
            ...
            else
            {
                ...
                this._logger.Info("There are {0} journey in cache... Exporting...", this._cachedRoutes.Count);
                ...
                AppContext.ExecuteTask(
                    ...
                    ProgressStyle.Marquee, 
                    this._logger, 
                    callback => { this._archiveManager.ExportData(this._cachedRoutes, this._dataPath, DataFormat.Binary, callback); });
                this._cachedRoutes.Clear();
            }
        }

        /// <summary>The finalize data.</summary>
