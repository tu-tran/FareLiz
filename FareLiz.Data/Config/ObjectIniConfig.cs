namespace SkyDean.FareLiz.Data.Config
{
    ...
    {
        /// <summary>The bind flgas.</summary>
        private readonly BindingFlags bindFlgas = BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic;
        ...
        /// <summary>Gets or sets the ini file path.</summary>
                ...
                if (prop.CanRead && prop.CanWrite)
                {
                    ...
                    foreach (var attrib in attributes)
                    {
                        ...
                        if (configAttrib != null)
                        {
                            ...
                            {
                                var propKey = string.IsNullOrEmpty(configAttrib.ConfigurationKey) ? prop.Name : configAttrib.ConfigurationKey;
                                var storedValue = this.GetKeyValue(section.Name, propKey);
                ...
                if (prop.CanRead && prop.CanWrite)
                {
                    ...
                    foreach (var attrib in attributes)
                    {
                        ...
                        if (configAttrib != null)
                        {
                            if (configAttrib.IsConfigurable)
                            {
                                var keyStr = string.IsNullOrEmpty(configAttrib.ConfigurationKey) ? prop.Name : configAttrib.ConfigurationKey;
                                ...
                                var val = prop.GetValue(targetObject, null);
                                if (val != null)
                                {
