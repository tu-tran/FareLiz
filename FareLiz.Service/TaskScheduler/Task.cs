...
{
    using SkyDean.FareLiz.Service.TaskScheduler.V1;
    using SkyDean.FareLiz.Service.TaskScheduler.V2;
    ...
    /// <summary>Defines what kind of Terminal Server session RequestState change you can use to trigger a task to start. These changes are used to specify the type of RequestState change in the SessionStateChangeTrigger.</summary>
    public enum TaskSessionStateChangeType
    {
        /// <summary>Terminal Server console connection RequestState change. For example, when you connect to a user session on the local computer by switching users on the computer.</summary>
        ConsoleConnect = 1,
        /// <summary>Terminal Server console disconnection RequestState change. For example, when you disconnect to a user session on the local computer by switching users on the computer.</summary>
        ConsoleDisconnect = 2,
        /// <summary>Terminal Server remote connection RequestState change. For example, when a user connects to a user session by using the Remote Desktop Connection program from a remote computer.</summary>
        RemoteConnect = 3,
        /// <summary>Terminal Server remote disconnection RequestState change. For example, when a user disconnects from a user session while using the Remote Desktop Connection program from a remote computer.</summary>
        RemoteDisconnect = 4,
        /// <summary>Terminal Server session locked RequestState change. For example, this RequestState change causes the task to run when the computer is locked.</summary>
        SessionLock = 7,
        /// <summary>Terminal Server session unlocked RequestState change. For example, this RequestState change causes the task to run when the computer is unlocked.</summary>
        SessionUnlock = 8
    }
    ...
    */
    ...
    {
        /// <summary>The RequestState of the task is unknown.</summary>
        Unknown,
    ...
    public sealed class IdleSettings : IDisposable
    {
        ...
        /// <summary>
        /// Gets or sets a value that indicates the amount of time that the computer must be in an idle RequestState before the task is run.
        /// </summary>
    ...
    {
        private IMaintenanceSettings v2Settings = null;
    ...
    {
        private INetworkSettings v2Settings = null;
    ...
    public sealed class RunningTask : Task
    {
        ...
        /// <summary>
        /// Gets the operational RequestState of the running task.
        /// </summary>
    ...
    public class Task : IDisposable
    {
        ...
        /// <summary>
        /// Gets the operational RequestState of the registered task.
        /// </summary>
        ...
        public DateTime[] GetRunTimes(DateTime start, DateTime end, uint count = 0)
        {
            ...
            SystemTime stStart = new SystemTime(start);
            SystemTime stEnd = new SystemTime(end);
            ...
            int stSize = Marshal.SizeOf(typeof(SystemTime));
            for (int i = 0; i < count; i++)
            {
        ...
        /// <remarks>The SkyDean.FareLiz.Service.TaskSchedulerEditor.dll assembly must reside in the same directory as the SkyDean.FareLiz.Service.TaskScheduler.dll or in the GAC.</remarks>
        public bool ShowEditor()
        {
            try
            {
                System.Reflection.Assembly asm = System.Reflection.Assembly.LoadFrom("SkyDean.FareLiz.Service.TaskSchedulerEditor.dll");
                if (asm != null)
                {
                    Type t = asm.GetType("SkyDean.FareLiz.Service.TaskScheduler.TaskEditDialog", false, false);
                    if (t != null)
                    {
        ...
        {
            ITask iTask = TaskService.GetTask(this.TaskService.v1TaskScheduler, GetV1Path(v1Task));
            if (iTask != null)
                v1Task = iTask;
        }
        ...
        internal static TimeSpan StringToTimeSpan(string input)
        {
            if (!string.IsNullOrEmpty(input))
                try { return System.Xml.XmlConvert.ToTimeSpan(input); }
                catch { }
            return TimeSpan.Zero;
        }
        ...
        internal static string TimeSpanToString(TimeSpan span)
        {
            if (span != TimeSpan.Zero)
                try { return System.Xml.XmlConvert.ToString(span); }
                catch { }
            return null;
        }
    }
    ...
    public sealed class TaskDefinition : IDisposable, IXmlSerializable
    {
        ...
        internal ITask v1Task = null;
        internal ITaskDefinition v2Def = null;
        ...
        public static XmlSchemaComplexType GetV1SchemaFile(XmlSchemaSet xs)
        {
            ...
            using (System.IO.Stream xsdFile = System.Reflection.Assembly.GetAssembly(typeof(TaskDefinition)).GetManifestResourceStream("SkyDean.FareLiz.Service.TaskScheduler.V1.TaskSchedulerV1Schema.xsd"))
            {
    ...
    {
        private ITask v1Task = null;
        private IRegistrationInfo v2RegInfo = null;
    ...
    public sealed class TaskSettings : IDisposable, IXmlSerializable
    {
        ...
        private ITask v1Task = null;
        private ITaskSettings v2Settings = null;
        private ITaskSettings2 v2Settings2 = null;
        private ITaskSettings3 v2Settings3 = null;
        ...
        /// <summary>
        /// Gets or sets the information that specifies how the Task Scheduler performs tasks when the computer is in an idle RequestState.
        /// </summary>
