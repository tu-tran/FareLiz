...
namespace SkyDean.FareLiz.Service.TaskScheduler
{
	...
	public sealed class TaskService : Component, IDisposable, ISupportInitialize
	{
		...
		internal ITaskScheduler v1TaskScheduler = null;
		internal TaskSchedulerClass v2TaskService = null;
		...
		public Task GetTask(string taskPath)
		{
			...
			if (v2TaskService != null)
			{
				IRegisteredTask iTask = GetTask(this.v2TaskService, taskPath);
				if (iTask != null)
					t = new Task(this, iTask);
			}
			else
			{
				ITask iTask = GetTask(this.v1TaskScheduler, taskPath);
				if (iTask != null)
					t = new Task(this, iTask);
			}
			return t;
		...
		public TaskDefinition NewTask()
		{
			...
			Guid ITaskGuid = Marshal.GenerateGuidForType(typeof(ITask));
			Guid CTaskGuid = Marshal.GenerateGuidForType(typeof(CTask));
			...
			if (!initializing && !DesignMode)
			{
				...
				(string.IsNullOrEmpty(userDomain) && string.IsNullOrEmpty(userName) && string.IsNullOrEmpty(userPassword))))
				{
					...
					else
					{
						...
						CTaskScheduler csched = new CTaskScheduler();
