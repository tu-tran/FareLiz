namespace SkyDean.FareLiz.Service.Versioning
{
    ...
    public class OnlineVersionRetriever : IVersionRetriever
    {
        ...
        /// <summary>The _manifest urls.</summary>
        private readonly string[] _manifestUrls = { "http://skydean.com/Repository/" + AppUtil.ProductName };
        ...
        /// <summary>The get current version.</summary>
        ...
        /// <summary>The get latest version.</summary>
        /// <returns>The <see cref="VersionInfo" />.</returns>
        public VersionInfo GetLatestVersion()
        {
            foreach (var host in this._manifestUrls)
            {
                    ...
                    using (var response = (HttpWebResponse)request.GetResponse())
                    {
                        if (response.StatusCode == HttpStatusCode.OK && response.ContentType != "text/html")
                        {
                            ...
                            using (var reader = new StreamReader(stream, Encoding.Default))
                            {
                                ...
                                var x86pkg = this.ProcessUri(request.RequestUri, x86);
                                var x64pkg = this.ProcessUri(request.RequestUri, x64);
                ...
                {
                    this.Logger.Error("Failed to get latest version info [{0}]: {1}", host, ex.Message);
                }
            }
        ...
        /// <summary>The check for update.</summary>
        /// <returns>The <see cref="UpdateRequest" />.</returns>
        ...
        public void DownloadPackage(UpdateRequest request, string targetLocation)
        {
            ...
            var pkgLocation = isX64 ? request.ToVersion.X64Package : request.ToVersion.X86Package;
        ...
        {
            var baseUrl = manifestUri.Scheme + "://" + manifestUri.Host;
            if (!packageLocation.StartsWith("/"))
            {
                // Absolute path
                for (var i = 0; i < manifestUri.Segments.Length - 1; i++)
                {
                    baseUrl += manifestUri.Segments[i];
                }
            }

            var result = baseUrl + packageLocation;
            return result;
        }
    }
}