namespace SkyDean.FareLiz.Service.Utils
{
    ...
    public class FtpItem
    {
        ...
        /// <summary>
        /// List of REGEX formats for different FTP server listing formats The first three are various UNIX/LINUX formats, fourth is for MS FTP in detailed mode
        /// and the last for MS FTP in 'DOS' mode.
        /// </summary>
        ...
        {
            string directory;
            Match match = this.GetMatchingRegex(line); // Get the match of the current listing.
            if (match == null)
            {
            ...
            try
            {
                this._fileDateTime = DateTime.Parse(match.Groups["timestamp"].Value, this.culture); // Set the datetime of the listing.
            }
            catch
            {
        ...
        /// The culture.
        /// </summary>
        private IFormatProvider culture = CultureInfo.InvariantCulture; // Eliminate DateTime parsing issues.
