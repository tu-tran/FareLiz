namespace SkyDean.FareLiz.WinForm.Presentation.Controllers
{
    ...
    internal sealed class CheckFareController
    {
        ...
        {
            var minDuration = this.View.MinDuration;
            var maxDuration = this.View.MaxDuration;
            var result = new List<FareMonitorRequest>();
            var roundTrip = this.View.IsRoundTrip;
            var departureRange = this.View.DepartureDateRange;

            for (var i = -Convert.ToInt32(departureRange.Minus); i <= departureRange.Plus; i++)
            {
                var retStart = roundTrip ? -Convert.ToInt32(departureRange.Minus) : 0;
                var retEnd = roundTrip ? Convert.ToInt32(departureRange.Plus) : 0;
                for (var j = retStart; j <= retEnd; j++)
                {
                    var dept = this.View.DepartureDate.Date.AddDays(i);
                    var ret = roundTrip ? this.View.ReturnDate.Date.AddDays(j) : DateTime.MinValue;
                    var stayDuration = (int)(ret - dept).TotalDays;
        ...
        internal FareRequestMonitor Monitor(OperationMode mode)
        {
            ...
            AppContext.ExecuteTask(
                null, 
                "Fare Monitor", 
                "Starting fare monitor...", 
                this.GetType() + "-StartMonitor", 
                ...
                callback =>
                    {
                        try
                        {
                            ...
                            if (newRequests.Count > 0)
                            {
                                ...
                                AppContext.Logger.Info("{0}: Starting monitor for {1} new requests", this.GetType().Name, newRequests.Count);
                                this._monitors.Start(newMon);
                            }
                            else
                            {
                                var period = this.View.MinDuration == this.View.MaxDuration
...
Double-check the filter conditions and make sure that not all travel dates are in the past", 
                                ...
                                callback.Inform(null, message, "Invalid parameters", NotificationType.Exclamation);
                            }
                        }
                        catch (Exception ex)
                        {
                            callback.Inform(this.View, "Failed to start monitors: " + ex.Message, "Check Fares", NotificationType.Error);
