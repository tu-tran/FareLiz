namespace SkyDean.FareLiz.WinForm.Presentation.Views
{
    ...
    internal partial class SchedulerForm : SmartForm
    {
        ...
        {
            this.imgListSchedules.Images.Add("Add", Properties.Resources.Add);
            this.imgListSchedules.Images.Add("Edit", Properties.Resources.Edit);
            this.imgListSchedules.Images.Add("Error", Properties.Resources.Stop);
        }
        ...
        private void Reload()
        {
            ...
            var initalStatus = "Loading scheduled tasks...";
            this.lblStatus.Text = initalStatus;
            this.lblStatus.Image = Properties.Resources.Loading;
                ...
                o =>
                    {
                        try
                        {
                            ...
                            var tasks = this._scheduler.GetScheduledTasks();
                            this.SafeInvoke(
                                new MethodInvoker(
                                    () =>
                                        {
        ...
        private void btnDeleteAll_Click(object sender, EventArgs e)
        {
            if (this.lvSchedules.Items.Count > 0)
            {
                ...
                {
                    for (var i = 0; i < this.lvSchedules.Items.Count; i++)
                    {
                        var item = this.lvSchedules.Items[i].Tag as ScheduleItem;
                        if (item != null && item.IsCreated)
                        {
                            var taskName = string.IsNullOrEmpty(item.CreatedName) ? item.Setting.TaskName : item.CreatedName;
                            this._scheduler.RemoveTask(taskName);
                        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (itemTags.Count > 0)
            {
                var initialStatus = this.lblStatus.Text = "Saving scheduled tasks...";
                this.lblStatus.Image = Properties.Resources.Loading;
        ...
        {
            var selected = this.flightScannerProperties.SelectedObject != null;
        ...
        private void mnuCopyParamsToClipboard_Click(object sender, EventArgs e)
        {
            try
            {
                var args = this.GetExecutionParamArgs();
