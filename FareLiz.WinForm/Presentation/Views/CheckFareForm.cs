namespace SkyDean.FareLiz.WinForm.Presentation.Views
{
    ...
    internal partial class CheckFareForm : SmartForm
    {
        ...
        public CheckFareForm(ExecutionParam param)
        {
            ...
            this.fareBrowserTabs.ImageList = new ImageList { ImageSize = new Size(6, 6) };

            // Get extra empty space on tab header
        }
        ...
        {
            var skipDurationConstraint = this.numDepartDateRangePlus.Value == 0 && this.numDepartDateRangeMinus.Value == 0
        ...
        internal void SetScanner(bool enabled)
        {
            this.InvokeActionIfNeeded(
                (MethodInvoker)delegate { this.btnShowFare.Enabled = this.btnLiveMonitor.Enabled = this.btnGetFareAndSave.Enabled = enabled; });
        ...
        private IList<TravelRoute> ExtractTabData()
        {
            ...
            foreach (TabPage tab in this.fareBrowserTabs.TabPages)
            {
                try
                {
                    ...
                    {
                        var route = request.BrowserControl.LastRetrievedRoute;
                        if (route != null)
                        {
                ...
                {
                    AppContext.Logger.Error("Failed to get data from tab: " + ex);
                }
            }
        ...
        {
            var action = request.BrowserControl.RequestState == DataRequestState.Pending ? "Getting" : "Waiting";
            var oneWayTrip = request.ReturnDate.IsUndefined();
            var trip = string.Format(
                "{0} trip {1}{2}",
        ...
        {
            var minusWidth = (this.statusStrip.SizingGrip ? this.statusStrip.SizeGripBounds.Width : 0) + 5 + 2 * SystemInformation.BorderSize.Width
            ...
            var newWidth = this.statusStrip.Width - minusWidth;
            if (newWidth > 0)
            {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                AppContext.ExecuteTask(
                    ...
                    AppContext.Logger,
                    callback => { db.AddData(tabRoutes, callback); });
            }
            finally
            {
                ...
                else
                {
                    ...
                    AppContext.ExecuteTask(
                        ...
                        callback =>
                        {
                            ...
                            progress.Begin();
                            pkgId = syncDb.SendData(journeys, callback);
                ...
                (MethodInvoker)delegate
                    {
                        ...
                        // Initialize request here: So that the control is created on the UI thread
                        var browserTabPage =
                            new TabPage(
        ...
        {
            this.InvokeActionIfNeeded((MethodInvoker)delegate { MessageBox.Show(this, message, title, MessageBoxButtons.OK, icon); });
        }
    }
}