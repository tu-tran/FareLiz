namespace SkyDean.FareLiz.WinForm.Presentation.Views
{
    ...
    internal sealed partial class FlightStatisticForm : SmartForm
    {
        ...
        private void InitializeView()
        {
            ...
            var isDbConfigured = this.FareDatabase != null;
        ...
        private void FillDateComboBox()
        {
            ...
            try
            {
                ...
                var deptDateList = this._activeRoute.Journeys.DistinctBy(o => o.DepartureDate).Select(o => o.DepartureDate).OrderBy(o => o);
                var retDateList = this._activeRoute.Journeys.DistinctBy(o => o.ReturnDate).Select(o => o.ReturnDate).OrderBy(o => o);

                foreach (var d in deptDateList)
                {
                    this.cbDepartureDate.Items.Add(d.ToString(STR_FORMAT_DATE_COMBO));
                }

                var retCount = 0;
                foreach (var d in retDateList)
                {
                ...
                this.cbDepartureDate.Items.Add(string.Empty);

                // Add an empty row to the end (this allows user to make the filter "undefined"
        ...
        /// The populate Data.
        /// </param>
        private void BindListViewAsync(TravelRoute route,
        ...
        /// The populate data.
        /// </param>
        private void BindListView(TravelRoute route,
                                  DateTime departureDate,
                                  DateTime returnDate,
                                  double priceLimit,
                                  bool viewAllHistory,
                                  ...
                                  bool populateData)
        {
            ...
            try
            {
                    ...
                    AppContext.ExecuteTask(
                        ...
                        callback =>
                        {
                            foreach (var j in journeyCollection)
                            {
                                ...
                                {
                                    this.FareDatabase.LoadData(loadBatch, viewAllHistory, callback);
                                    loadBatch.Clear();
                                }
                ...
                var totalJourneysCount = journeyCollection.Count();
                var totalFlightsCount = 0;
                ...
                var checkDuration = (departureDate.IsUndefined() || returnDate.IsUndefined()) && returnDate != DateTime.MinValue;
                foreach (var j in journeyCollection)
                {
                    ...
                    var skippedJourney = false;
                    if (checkDuration)
                    {
                        var stayDuration = (int)(j.ReturnDate.Date - j.DepartureDate.Date).TotalDays;
                        if (stayDuration < 0)
                        {
                            // One-way trip
                            stayDuration = 0;
                    ...
                    var journeyFlights = skippedJourney ? null : new List<Flight>();
                    foreach (var d in j.Data)
                    {
                        ...
                        else
                        {
                            ...
                            var taking = amountFlightsPerData > flights.Count ? flights.Count : amountFlightsPerData;
                    ...
                    foreach (var flight in journeyFlights)
                    {
                    ...
                    new Action(
                        () =>
                        {
                            ...
                            if (opearators == null)
                            {
                                ...
                                var sortedOps = newOperatorCheckList.OrderBy(p => p.Key).ToList();
                                ...
                                sortedOps.ForEach(
                                    p =>
                                    {
                                        var pos = this.cbAirlines.Items.Add(p.Key);
                            ...
                            {
                                var journeyStr = totalJourneysCount > 1 ? " (" + totalJourneysCount + " dates)" : null;
                                ...
                                var skipStr = parts.Count > 0 ? " | " + string.Join(", ", parts.ToArray()) : string.Empty;
            ...
            catch (Exception ex)
            {
                ...
                this.SafeInvoke(new Action(() => MessageBox.Show(this, ex.ToString(), "Could not display data")));
        ...
        private void FlightFilter_Changed(object sender, EventArgs e)
        {
            ...
            var reloadData = sender == this.cbDepartureDate || sender == this.cbReturnDate || sender == this;

            var condition = this.ReadFilter();
        ...
        {
            var departureDate = string.IsNullOrEmpty(this.cbDepartureDate.Text)
                                    ? DateTime.MinValue
                                    : DateTime.ParseExact(this.cbDepartureDate.Text, STR_FORMAT_DATE_COMBO, null);
            var returnDate = string.IsNullOrEmpty(this.cbReturnDate.Text)
                                 ? DateTime.MaxValue
                                 : (this.cbReturnDate.Text == STR_ONE_WAY
                                        ? DateTime.MinValue
                                        : DateTime.ParseExact(this.cbReturnDate.Text, STR_FORMAT_DATE_COMBO, null));

            var minDuration = decimal.ToInt32(this.numMinDuration.Value);
            var maxDuration = decimal.ToInt32(this.numMaxDuration.Value);
            var maxPrice = decimal.ToInt32(this.numPriceLimit.Value);
            ...
            try
            {
                if (this._isMainForm)
                {
                    AppContext.ExecuteTask(
                        ...
                        "Load route data",
                        "Please wait...",
                        this.GetType() + "-LoadRoutes",
                        ProgressStyle.Marquee,
                        AppContext.Logger,
                        callback => { this._routeData = this.FareDatabase.GetRoutes(false, false, false, false, callback); },
                        null,
                        null,
                        true,
                        false);
                ...
                this.SafeInvoke(
                    new Action(
                        () =>
                        {
                            ...
                            var noRoute = originList == null || originList.Count == 0;
        ...
        {
            this.InvokeActionIfNeeded(new Action(() => this.lblStatus.Text = text));
        }
            ...
            if (syncDb != null)
            {
                AppContext.ExecuteTask(
                    ...
                    callback =>
                    {
                        ...
                        var count = syncDb.ReceivePackages(callback);
                        if (count < 0)
                        {
                            count = 0;
                        }
                ...
                if (sender == this.cbDeparture)
                {
                    ...
                    if (this._loadingRoutes)
                    {
                        ...
                        if (this.cbDestination.SelectedItem == null && this.cbDestination.Items.Count > 0)
                        {
                            ...
                            this.cbDestination.SelectedItem = this.cbDestination.Items[0];
                ...
                if (this._isMainForm)
                {
                    ...
                    if (selDeparture != null && selDestination != null)
                    {
                        ...
                        foreach (var r in this._routeData)
                        {
                            ...
                            {
                                this._activeRoute = r;
                                this.Logger.Debug("Reloading routes [{0}]-[{1}]...", deptCode, destCode);

                                AppContext.ExecuteTask(
                                    null,
                                    "Load database",
                                    "Please wait...",
                                    this.GetType() + "-LoadData",
                                    ProgressStyle.Marquee,
                                    AppContext.Logger,
                                    callback => { this.FareDatabase.LoadData(this._activeRoute, false, false, false, callback); },
                                    null,
                                    null,
                                    true,
                                    false);
