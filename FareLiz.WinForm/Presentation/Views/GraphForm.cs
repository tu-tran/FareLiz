namespace SkyDean.FareLiz.WinForm.Presentation.Views
{
    ...
    internal partial class GraphForm : SmartForm
    {
        ...
        {
            var graphPane = this.graph.GraphPane;
        ...
        {
            var travelPeriod = new Dictionary<string, object>();
            var dataDate = new Dictionary<string, object>();
        ...
        private void RenderGraph()
        {
            ...
            {
                var j = pair.Key;
                var period = StringUtil.GetPeriodString(j.DepartureDate, j.ReturnDate);
                string colorHex;
                ...
                var curveColor = ColorTranslator.FromHtml(colorHex);
                var curve = new LineItem(period, pair.Value, curveColor, SymbolType.Square);
            ...
            try
            {
                    ...
                    foreach (var pair in this._graphData)
                    {
                        ...
                        if (isAdding)
                        {
                            ...
                            foreach (var d in journey.Data)
                            {
                                ...
                                {
                                    var cheapestFlight = d.Flights[0];
                ...
                else if (tagData is DatePeriod)
                {
                    ...
                    foreach (var pair in this._graphData)
                    {
                        ...
                        for (var i = 0; i < targetCurveList.Count; i++)
                        {
                            var curve = targetCurveList[i];
                            var tagPeriod = curve.Tag as DatePeriod;
        ...
        {
            var added = e.NewValue == CheckState.Checked;
            var tag = ((ListView)sender).Items[e.Index].Tag;
    ...
    internal class DateComparer : ListViewColumnSorter
    {
        ...
        {
            var dateX = DateTime.Parse(x.Text);
            var dateY = DateTime.Parse(y.Text);
