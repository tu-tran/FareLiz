namespace SkyDean.FareLiz.WinForm
{
    ...
    internal sealed class SchedulerManager
    {
        ...
        public List<ScheduleItem> GetScheduledTasks()
        {
            ...
            {
                var tc = ts.RootFolder.GetTasks(null);
                foreach (var task in tc)
                {
                    try
                    {
                            ...
                            if (action != null)
                            {
                                    ...
                                    StringComparison.InvariantCultureIgnoreCase) == 0)
                                {
                                    try
                                    {
                                        ...
                                        var activeTriggerDate = DateTime.MinValue;
                    ...
                    {
                        this._logger.Error("Could not load task [{0}]: {1}", task.Name, ex.Message);
                    }
                }
            }
        ...
        internal void Schedule(ScheduleItem item)
        {
            ...
            using (var ts = new TaskService())
            {
                ...
                var deptRangeStr = string.Format(
                    "{0}(+{1} -{2})", 
                ...
                var retRangeStr = item.Parameters.ReturnDate.IsUndefined()
                                      ? null
                                      : string.Format(
                                          "{0}(+{1} -{2})", 
                ...
                var stayDurationStr = item.Parameters.ReturnDate.IsUndefined()
                                          ? "0"
...
Price Limit: {7}", 
                ...
                var cmdLine = item.Parameters.GenerateCommandLine();
                td.Actions.Add(new ExecAction(CurrentExe, cmdLine, Path.GetDirectoryName(CurrentExe)));

                // Register the task in the root folder
                var taskName = AppUtil.ProductName + " " + item.Setting.TaskName;
        ...
        {
            var existTask = service.FindTask(taskName, false);
            if (existTask != null)
            {
