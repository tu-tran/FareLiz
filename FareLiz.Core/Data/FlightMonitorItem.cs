    ...
    public class FlightDisplayItemsCollection : List<FlightMonitorItem>
    {
        ...
        public string GenerateRtfReport(string fontName, int fontSize)
        {
            ...
            for (var i = 0; i < this.Count; i++)
            {
                ...
                var flightOperator = item.FlightData.Operator;
                if (flightOperator.Length > maxOperatorLength)
                {
                    maxOperatorLength = flightOperator.Length;
                }

                var operatorUrl = item.FlightData.TravelAgency == null ? null : item.FlightData.TravelAgency.Url;

                var newPrice = item.FlightData.Price.ToString(CultureInfo.InvariantCulture);
                ...
                {
                    var oldPrice = item.OldPrice.ToString(CultureInfo.InvariantCulture);
                    if (oldPrice.Length > maxOldPriceLength)
                    {
                        maxOldPriceLength = oldPrice.Length;
                    }

                    var colorCode = item.FlightStatus == FlightStatus.PriceDecreased ? "\\cf2 " : "\\cf1 ";
                    var status = item.FlightStatus == FlightStatus.PriceDecreased ? "▼" : "▲";
                    if (maxOpLength == 0)
                    {
            ...
            var maxLineLength = maxOperatorLength + 2 + maxNewPriceLength + maxOpLength + maxOldPriceLength;
            for (var i = 0; i < dataLines.Count; i++)
            {
                var strArray = dataLines[i];
                var newLine = string.Empty;

                var operatorLink = string.IsNullOrEmpty(strArray[2])
                                       ? strArray[1]
                                       : string.Format(
