...
namespace SkyDean.FareLiz.Core.Utils
{
    public class StringFormatter
    {
        ...
        /// </summary>
        /// <param name="seed">Cipher key as a hex string</param>
        public StringFormatter(string seed)
        {
            randomSource = new RNGCryptoServiceProvider();
            SetupKey(HexToByte(seed));
        }

        /// <summary>
        /// Constructor for byte key
        /// </summary>
        /// <param name="seed">Cipher key as a byte array</param>
        ...
        public string Tag(string pt)
        {
            if (!IVSet)
                SetRandomIV();
            return ByteToHex(InitVector) + ByteToHex(Tag(Encoding.UTF8.GetBytes(pt), 2));
        }
        ...
        public string Untag(string ct)
        {
            ...
            return Encoding.UTF8.GetString(Tag(HexToByte(ct.Substring(16)), 2)).Replace("\0", "");
        }
