...
namespace SkyDean.FareLiz.Core.Utils
{
    ...
    /// <summary>The string formatter.</summary>
    public class StringFormatter
    {
        #region "Global variables and constants"

        /// <summary>The rounds.</summary>
        ...
        // SBLOCKS
        /// <summary>The bf_s 0.</summary>
        private uint[] bf_s0;

        /// <summary>The bf_s 1.</summary>
        private uint[] bf_s1;

        /// <summary>The bf_s 2.</summary>
        private uint[] bf_s2;

        /// <summary>The bf_s 3.</summary>
        private uint[] bf_s3;

        /// <summary>The bf_ p.</summary>
        private uint[] bf_P;
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="StringFormatter"/> class. Constructor for hex key
        /// </summary>
        /// <param name="seed">
        /// Cipher key as a hex string
        /// </param>
        public StringFormatter(string seed)
        {
            this.randomSource = new RNGCryptoServiceProvider();
            this.SetupKey(this.HexToByte(seed));
        }

        /// <summary>
        /// Initializes a new instance of the <see cref="StringFormatter"/> class. Constructor for byte key
        /// </summary>
        /// <param name="seed">
        /// Cipher key as a byte array
        /// </param>
        public StringFormatter(byte[] seed)
        {
            this.randomSource = new RNGCryptoServiceProvider();
            this.SetupKey(seed);
        }
        ...
        public string Tag(string pt)
        {
            ...
            return this.ByteToHex(this.InitVector) + this.ByteToHex(this.Tag(Encoding.UTF8.GetBytes(pt), 2));
        }
        ...
        /// </exception>
        public byte[] Tag(byte[] text, int numThreads)
        {
            if (!this.IVSet)
            {
            ...
            for (int i = 0; i < plainText.Length; i += 8)
            {
                for (int x = 0; x < 8; x++)
                {
                    input[x] = (byte)(counter[x] ^ this.InitVector[x]);
                }

                Buffer.BlockCopy(plainText, i, block, 0, 8);
                this.BlockEncrypt(ref input);
                this.XorBlock(ref block, input);
        ...
        /// The plaintext
        /// </returns>
        public string Untag(string ct)
        {
            this.IV = this.HexToByte(ct.Substring(0, 16));
            return Encoding.UTF8.GetString(this.Tag(this.HexToByte(ct.Substring(16)), 2)).Replace("\0", string.Empty);
        }

        /// <summary>Initialization vector for CBC mode.</summary>
        public byte[] IV
        {
            get
            {
        ...
        /// <summary>Gets or sets a value indicating whether non standard.</summary>
        ...
        /// <summary>Creates and sets a random initialization vector.</summary>
        ...
        /// </summary>
        /// <param name="cipherKey">
        /// Block cipher key (1-448 bits)
        /// </param>
        private void SetupKey(byte[] cipherKey)
        {
            ...
            {
                throw new Exception("ConfiguredType too long. 56 bytes required.");
            }
            ...
            for (int i = 0; i < 18; i++)
            {
                uint d =
                    (uint)
                    (((this.key[j % cipherKey.Length] * 256 + this.key[(j + 1) % cipherKey.Length]) * 256 + this.key[(j + 2) % cipherKey.Length])
                     * 256 + this.key[(j + 3) % cipherKey.Length]);
                this.bf_P[i] ^= d;
            ...
            {
                this.encipher();
                this.bf_s0[i] = this.xl_par;
                this.bf_s0[i + 1] = this.xr_par;
            }

            for (int i = 0; i < 256; i += 2)
            {
                this.encipher();
                this.bf_s1[i] = this.xl_par;
                this.bf_s1[i + 1] = this.xr_par;
            }

            for (int i = 0; i < 256; i += 2)
            {
                this.encipher();
                this.bf_s2[i] = this.xl_par;
                this.bf_s2[i + 1] = this.xr_par;
            }

            for (int i = 0; i < 256; i += 2)
            {
                this.encipher();
                this.bf_s3[i] = this.xl_par;
                this.bf_s3[i + 1] = this.xr_par;
            }
        }
        ...
        private void SetBlock(byte[] block)
        {
            ...
            {
                this.xr_par = BitConverter.ToUInt32(block1, 0);
                this.xl_par = BitConverter.ToUInt32(block2, 0);
            }
            else
            {
                ...
                this.xl_par = BitConverter.ToUInt32(block1, 0);
                this.xr_par = BitConverter.ToUInt32(block2, 0);
            }
        }
        ...
        /// </summary>
        /// <param name="block">
        /// 64 bit buffer to receive the block
        /// </param>
        private void GetBlock(ref byte[] block)
        {
            ...
            if (this.nonStandardMethod)
            {
                block1 = BitConverter.GetBytes(this.xr_par);
                block2 = BitConverter.GetBytes(this.xl_par);
            }
            else
            {
                block1 = BitConverter.GetBytes(this.xl_par);
                block2 = BitConverter.GetBytes(this.xr_par);
        ...
        /// <summary>Runs the blowfish algorithm (standard 16 rounds)</summary>
        private void encipher()
        {
            this.xl_par ^= this.bf_P[0];
            for (uint i = 0; i < ROUNDS; i += 2)
            {
                this.xr_par = this.round(this.xr_par, this.xl_par, i + 1);
                this.xl_par = this.round(this.xl_par, this.xr_par, i + 2);
            }

            this.xr_par = this.xr_par ^ this.bf_P[17];

            // swap the blocks
            uint swap = this.xl_par;
        ...
        {
            uint x1 = (this.bf_s0[this.wordByte0(b)] + this.bf_s1[this.wordByte1(b)]) ^ this.bf_s2[this.wordByte2(b)];
            uint x2 = x1 + this.bf_s3[this.wordByte3(b)];
            uint x3 = x2 ^ this.bf_P[n];
            return x3 ^ a;
        }
        ...
        public byte[] HexToByte(string hex)
        {
            ...
            {
                byte a = this.GetHex(hex[i]);
                byte b = this.GetHex(hex[i + 1]);
                r[i / 2] = (byte)(a * 16 + b);
            }
