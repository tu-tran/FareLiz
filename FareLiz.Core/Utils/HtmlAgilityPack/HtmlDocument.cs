...
namespace SkyDean.FareLiz.Core.Utils.HtmlAgilityPack
{
    ...
    public partial class HtmlDocument
    {
        ...
        public Encoding DetectEncoding(string path)
        {
            ...
            StreamReader sr = new StreamReader(path, this.OptionDefaultStreamEncoding);
        ...
        public void Load(TextReader reader)
        {
            ...
            if (!this.OptionCheckSyntax || this.Openednodes == null) return;
            ...
            if (prev == null)
            {
                if (HtmlNode.IsClosedElement(this._currentnode.Name))
                {
                    ...
                    if (this._lastparentnode != null)
                    {
                        ...
                        for (HtmlNode node = this._lastparentnode.LastChild; node != null; node = node.PreviousSibling)
                        {
                    ...
                    else
                    {
                        if (HtmlNode.IsEmptyElement(this._currentnode.Name))
                        {
                            this.AddError(
                                ...
                                this._currentnode._line, this._currentnode._lineposition,
                                this._currentnode._streamposition, this._currentnode.OuterHtml,
        ...
        {
            HtmlNode resetter = Utilities.GetDictionaryValueOrNull(this.Lastnodes, this._currentnode.Name);
            if (resetter == null)
                return null;
        ...
        private bool FindResetterNodes(HtmlNode node, string[] names)
        {
            ...
            {
                if (this.FindResetterNode(node) != null)
                    return true;
            }
            return false;
        }
        ...
        private void FixNestedTag(string name, string[] resetters)
        {
            ...
            if (prev == null || (this.Lastnodes[name].Closed)) return;
                                               ...
                                               "</" + this._currentnode.Name, StringComparison.OrdinalIgnoreCase) == 0)
                            {
                                ...
                                if ((c == '>') || (IsWhiteSpace(c)))
                                {
                                    ...
                                    HtmlNode script = this.CreateNode(HtmlNodeType.Text,
                                                                 this._currentnode._outerstartindex +
                                                                 this._currentnode._outerlength);
        ...
        private bool PushNodeEnd(int index, bool close)
        {
            ...
            if ((close) || (!this._currentnode._starttag))
            {
                if ((this.OptionStopperNodeName != null) && (this._remainder == null) &&
                    (string.Compare(this._currentnode.Name, this.OptionStopperNodeName, StringComparison.OrdinalIgnoreCase) == 0))
                {
                        ...
                        if (this._declaredencoding != null)
...
#endif
                            {
                                this.AddError(
                                    ...
                                    this._line, this._lineposition,
                                    this._index, node.OuterHtml,
                                    "Encoding mismatch between StreamEncoding: " +
                                    this._streamencoding.WebName + " and DeclaredEncoding: " +
                                    this._declaredencoding.WebName);
                            }
                    }
                }
