...
namespace SkyDean.FareLiz.Core.Utils.HtmlAgilityPack
{
    ...
    public partial class HtmlWeb
    {
        ...
        {
            get { return this._cachePath != null && this._usingCache; }
            set
            {
        ...
        public string GetCachePath(Uri uri)
        {
            ...
            if (uri.AbsolutePath == "/")
            {
                cachePath = Path.Combine(this._cachePath, ".htm");
            }
            else
            {
                cachePath = Path.Combine(this._cachePath, (uri.Host + uri.AbsolutePath).Replace('/', '\\'));
            }
            return cachePath;
        }
        ...
        public HtmlDocument Load(string url, string proxyHost, int proxyPort, string userId, string password)
        {
            ...
            return this.Load(url, "GET", myProxy, myCreds);
        }
        ...
        public HtmlDocument Load(string url, string method)
        {
            ...
            if ((uri.Scheme == Uri.UriSchemeHttps) ||
                (uri.Scheme == Uri.UriSchemeHttp))
            {
                doc = this.LoadUrl(uri, method, null, null);
            }
            else
            {
                if (uri.Scheme == Uri.UriSchemeFile)
                {
                    ...
                    doc.DetectEncodingAndLoad(url, this._autoDetectEncoding);
                }
                else
                {
                    throw new HtmlWebException("Unsupported uri scheme: '" + uri.Scheme + "'.");
        ...
        public HtmlDocument Load(string url, string method, WebProxy proxy, NetworkCredential credentials)
        {
            ...
            if ((uri.Scheme == Uri.UriSchemeHttps) ||
                (uri.Scheme == Uri.UriSchemeHttp))
            {
                doc = this.LoadUrl(uri, method, proxy, credentials);
            }
            else
            {
                if (uri.Scheme == Uri.UriSchemeFile)
        ...
        {
            HtmlDocument doc = this.Load(htmlUrl);
            doc.Save(writer);
        }
                                   ...
                                   ICredentials creds)
        {
            ...
            if (this.UsingCache)
            {
                cachePath = this.GetCachePath(req.RequestUri);
            ...
            {
                this._requestDuration = Environment.TickCount - tc;
            ...
            {
                this.PostResponse(req, resp);
            }

            this._requestDuration = Environment.TickCount - tc;
            this._responseUri = resp.ResponseUri;

            bool html = this.IsHtmlContent(resp.ContentType);
            ...
            if (s != null)
            {
                if (this.UsingCache)
                {
                    ...
                    SaveStream(s, cachePath, RemoveMilliseconds(resp.LastModified), this._streamBufferSize);

                    // save headers
                    this.SaveCacheHeaders(req.RequestUri, resp);
        ...
        private string GetCacheHeader(Uri requestUri, string name, string def)
        {
            ...
            doc.Load(this.GetCacheHeadersPath(requestUri));
            XmlNode node =
        ...
        private void SaveCacheHeaders(Uri requestUri, HttpWebResponse resp)
        {
            ...
            string file = this.GetCacheHeadersPath(requestUri);
            XmlDocument doc = new XmlDocument();
            doc.LoadXml("<c></c>");
