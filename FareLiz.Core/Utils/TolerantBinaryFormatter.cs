        ...
        private class TolerantBinder : SerializationBinder
        {
            ...
            public override Type BindToType(string assemblyName, string typeName)
            {
                ...
                if (result == null)
                {
                    ...
                    var tolerantAsmName = asmName.Name;
                    var typeNameParts = typeName.Split('.');
                    var tolerantTypeName = typeNameParts[typeNameParts.Length - 1];
                    ...
                    foreach (var asm in loadedAssemblies)
                    {
                        ...
                        var matchAsm = string.Equals(asm.GetName().Name, tolerantAsmName, StringComparison.OrdinalIgnoreCase);
                        foreach (var type in asmTypes)
                        {
                            if (type.FullName == typeName)
                            {
                                if (matchAsm)
                                {
                                    ...
                                    // If we found a type with exact same signature: No need to look further
                                    break;
                                }

                                diffAsmExactType = type;

                                // We get a type with exact same signature on different assembly
                            }
