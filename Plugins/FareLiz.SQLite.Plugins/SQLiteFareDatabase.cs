namespace SkyDean.FareLiz.SQLite
{
    ...
    /// <summary>The sq lite fare database.</summary>
    ...
    public sealed partial class SQLiteFareDatabase : ISyncableDatabase
    {
        ...
        /// <summary>The datetim e_ format.</summary>
        ...
        /// <summary>Gets the data file name.</summary>
        ...
        /// <summary>Gets or sets the data synchronizer.</summary>
        public IDataSyncer DataSynchronizer { get; set; }

        /// <summary>Gets or sets the package synchronizer.</summary>
            ...
            using (var connection = new SQLiteConnection(this._connectionString))
            {
                using (var getPlacesCmd = new SQLiteCommand("SELECT LID, SDEPARTURE, SDESTINATION FROM ROUTE", connection))
                {
                    ...
                    using (var reader = getPlacesCmd.ExecuteReader())
                    {
                        if (reader.HasRows)
                        {
                            var iLid = reader.GetOrdinal("LID");
                            var iDeparture = reader.GetOrdinal("SDEPARTURE");
                            var iDestination = reader.GetOrdinal("SDESTINATION");

                            while (reader.Read())
                            {
                                var id = reader.GetInt64(iLid);
                                var origin = reader.GetString(iDeparture);
                                var destination = reader.GetString(iDestination);
        ...
        public void LoadData(TravelRoute route, bool loadJourneyData, bool loadHistory, bool loadFlights, IProgressCallback callback)
        {
            ...
            using (var connection = new SQLiteConnection(this._connectionString))
            {
                ...
                var selectSql = "SELECT J.LID, J.TDEPARTURE, J.TRETURN"
                                + (loadJourneyData
                                       ? (", D.LID DATAID, D.SCURRENCY, " + (loadHistory ? "D.TUPDATE" : "MAX(D.TUPDATE) TUPDATE")
                                          + (loadFlights ? ", D.BFLIGHT" : string.Empty))
                                       : string.Empty) + " FROM JOURNEY J" + (loadJourneyData ? ", JOURNEY_DATA D " : string.Empty)
                                + " WHERE J.LROUTEID = @lRouteId "
                                + (loadJourneyData
                                       ? " AND D.LJOURNEYID = J.LID " + (loadHistory ? string.Empty : " GROUP BY (D.LJOURNEYID) ")
                                       : string.Empty);
                ...
                using (var getJourneyCmd = new SQLiteCommand(selectSql, connection))
                {
                    ...
                    using (var reader = getJourneyCmd.ExecuteReader())
                    {
                        if (reader.HasRows)
                        {
                            var iId = reader.GetOrdinal("LID");
                            var iDeparture = reader.GetOrdinal("TDEPARTURE");
                            var iReturn = reader.GetOrdinal("TRETURN");
                            var iUpdate = reader.GetOrdinal("TUPDATE");
                            var iCurrency = loadJourneyData ? reader.GetOrdinal("SCURRENCY") : -1;
                            var iDataId = loadJourneyData ? reader.GetOrdinal("DATAID") : -1;
                            var iFlight = loadJourneyData ? reader.GetOrdinal("BFLIGHT") : -1;

                            while (reader.Read())
                            {
                                var journeyId = reader.GetInt64(iId);
        ...
        {
            this.Logger.Debug("Load data for {0} journeys [{1}]", journeys.Count, loadHistory ? "H" : null);
            foreach (var j in journeys)
            {
                j.Data.Clear();
            }

            var condition = GenerateCondition(journeys);
            using (var connection = new SQLiteConnection(this._connectionString))
            {
                var selectSql = "SELECT LID, LJOURNEYID, SCURRENCY, BFLIGHT, " + (loadHistory ? "TUPDATE" : "MAX(TUPDATE) TUPDATE")
                ...
                using (var getFlightsCmd = new SQLiteCommand(selectSql, connection))
                {
                    ...
                    using (var reader = getFlightsCmd.ExecuteReader())
                    {
                        if (reader.HasRows)
                        {
                            ...
                            {
                                var iId = reader.GetOrdinal("LID");
                                var iJourneyId = reader.GetOrdinal("LJOURNEYID");
                                var iCurrency = reader.GetOrdinal("SCURRENCY");
                                var iUpdate = reader.GetOrdinal("TUPDATE");
                                var iFlight = reader.GetOrdinal("BFLIGHT");
                                ...
                                while (reader.Read())
                                {
                                    ...
                                    {
                                        var dataId = reader.GetInt64(iId);
                                        var journeyId = reader.GetInt64(iJourneyId);
                                        var currency = reader.GetString(iCurrency);
                                        var dataDateStr = reader.GetString(iUpdate);
                                        var dataDate = DateTime.ParseExact(
                                            dataDateStr, 
                                            DATETIME_FORMAT, 
                                            CultureInfo.InvariantCulture, 
        ...
        public ValidateResult ValidateDatabase(IProgressCallback callback)
        {
            ...
            using (var connection = new SQLiteConnection(this._connectionString))
            {
                ...
                {
                    connection.Open();
                    var result = getFlightCmd.ExecuteScalar();
...
Packages: {4}", this.DataFileName, StringUtil.FormatSize(new FileInfo(this.DataFileName).Length), route, journey, package);
        ...
        private void DoResetDatabase(IProgressCallback callback)
        {
            ...
            string sql;
            using (var dataStream = this.GetType().Assembly.GetManifestResourceStream(this.GetType().Namespace + "." + "DbSchema.sqlite"))
            {
                if (dataStream == null)
                {
                ...
                using (var sr = new StreamReader(dataStream)) sql = sr.ReadToEnd();
            }
        ...
        private void DoAddData(IList<TravelRoute> data, string packageId, DateTime packageCreatedAt, ProgressDialog callback)
        {
            using (var connection = new SQLiteConnection(this._connectionString))
            {
                ...
                {
                    long totalData = 0;
                    this.Logger.Info("Add new {0} travel routes into database", data.Count);
                    ...
                    var totalJourneys = data.Sum(r => r.Journeys.Count);
                    ...
                    using (insertJourneyDataCmd)
                    {
                        foreach (var r in data)
                        {
                            ...
                            var isNewRoute = dbRouteId == null || dbRouteId is DBNull;
                            if (isNewRoute)
                            {
                            ...
                            foreach (var j in r.Journeys)
                            {
                                ...
                                foreach (var journeyData in j.Data)
                                {
                                    var isNewJourneyData = true;
                                    if (!isNewJourney)
                                    {
                    ...
                    this.Logger.Info("Completed adding {0} journey data. Commiting", totalData);
        ...
        private bool IsPackageImported(string packageId, SQLiteConnection connection, IProgressCallback callback)
        {
            ...
            {
                var result = getPackageCountCmd.ExecuteScalar();
                if (!(result == null || result is DBNull))
                {
        ...
        /// <summary>The try open database.</summary>
