namespace SkyDean.FareLiz.IO
{
    ...
    public class FileArchiveManager : IArchiveManager
    {
        ...
        public string ExportData(IList<TravelRoute> data, string destinationPath, DataFormat format, IProgressCallback callback)
        {
            ...
            this.Logger.Info("Export {0} travel route", data.Count);
            ...
            this.Logger.Info("{0} travel route was exported", data.Count);
            return targetFile;
        }
        ...
        public IList<TravelRoute> ImportData(string path, DataOptions options, IProgressCallback callback)
        {
            ...
            for (var i = 0; i < acceptedFiles.Count; i++)
            {
                ...
                var ext = Path.GetExtension(fileName); // Get file extension in order to determine the data type

                this.Logger.Info("Importing file [{0}]", fileName);
        ...
        {
            var origin = route.Departure.IATA;
            var dest = route.Destination.IATA;
