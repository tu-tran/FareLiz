namespace SkyDean.FareLiz.InterFlight
{
    ...
    /// <summary>The pt data parser.</summary>
    internal class PTDataParser
    {
        /// <summary>The _root domain.</summary>
        ...
        /// <summary>Gets or sets the max flights per airline.</summary>
        internal int MaxFlightsPerAirline { get; set; }

        /// <summary>Gets or sets the max airlines.</summary>
        internal int MaxAirlines { get; set; }

        /// <summary>Gets or sets the min price margin.</summary>
        ...
        internal RouteDataResult ParseWebArchive(HtmlDocument webDocument)
        {
            ...
            var originInput = webDocument.GetElementbyId("text_fly_from");
            var origin = originInput.GetAttributeValue("value", string.Empty);
            var destInput = webDocument.GetElementbyId("text_fly_to");
            var destination = destInput.GetAttributeValue("value", string.Empty);
            ...
            foreach (var resultSection in resultDivs)
            {
                ...
                {
                    var dataDateStr = dataDateAttrib.Value;
                ...
                foreach (var flightNode in resultElements)
                {
                    ...
                    foreach (var flightDetailNode in colNodes)
                    {
                        ...
                        var className = flightDetailNode.GetAttributeValue("class", string.Empty);
                        switch (className)
                        {
                            case "f_outbound":
                            ...
                            case "f_return":
                                ...
                                if (divNodes != null)
                                {
                                    ...
                                    {
                                        // Each flight
                                        var dataClass = dataNode.GetAttributeValue("class", string.Empty);
                                        switch (dataClass)
                                        {
                                            case "f_dep_date":
                                            ...
                                            out arrDate) && arrDate.IsDefined())
                                    {
                                        ...
                                        {
                                            int transit;
                                            int.TryParse(stopStr, out transit);

                                            // This might fail for straight flight: Just ignore it
                            ...
                            case "f_price":
                                var priceStr = Regex.Match(flightDetailNode.InnerText.Replace(",", "."), @"\d+(.\d+)?").Value;
                    ...
                    if (outboundLeg != null)
                    {
                        ...
                        if (!shouldAdd)
                        {
                            ...
                            var totalDuration = (outboundLeg == null ? TimeSpan.Zero : outboundLeg.Duration)
                            ...
                            else
                            {
                                ...
                                for (var i = opearatorFlights.Count - 1; i >= 0; i--)
                                {
                ...
                if (newData.Flights.Count > 0)
                {
                    ...
                    var deptDate = newData.Flights[0].OutboundLeg.Departure.Date;
        ...
        private TimeSpan TryGetDuration(string input)
        {
            ...
            if (match.Success)
            {
                ...
                var totalMin = int.Parse(hourStr) * 60;
        ...
        private TravelAgency TryGetTravelAgency(string input, bool forceRetrieveActualUrl)
        {
            ...
            if (match.Success)
            {
                ...
                if (!string.IsNullOrEmpty(agency))
                {
                    ...
                    var result = new TravelAgency(agency);
