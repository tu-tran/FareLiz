namespace SkyDean.FareLiz.WinForm.Components.Controls.DatePicker
{
    ...
    internal class MonthCalendarDate : ICloneable
    {
        ...
        public MonthCalendarDate FirstOfMonth
        {
            get
            {
                if (this._firstOfMonth == null)
                {
                    if (this.Date == this._calendar.MinSupportedDateTime.Date)
                    {
                        this._firstOfMonth = this.Clone() as MonthCalendarDate;
                    }
                    else
                    {
        ...
        {
            var firstDayOfWeek = provider.FirstDayOfWeek;
            var dt = (MonthCalendarDate)this.Clone();
        ...
        {
            var currentDayOfWeek = this.DayOfWeek;
            var firstDayOfWeek = provider.FirstDayOfWeek;
        ...
        public MonthCalendarDate AddDays(int days)
        {
            ...
            return new MonthCalendarDate(this._calendar, dt);
        }

        /// <summary>Returns a string representation of the native <see cref="Calendar" /> dependent date using the current UI's
        /// <see cref="DateTimeFormatInfo.ShortDatePattern" />.</summary>
        ...
        /// <summary>
        /// Returns a string representation of the native <see cref="Calendar"/> dependent date using the
        /// <see cref="IFormatProvider"/> <see cref="DateTimeFormatInfo"/>
        ...
        /// <summary>
        /// Returns a string representation of the native <see cref="Calendar"/> dependent date using the
        /// <see cref="IFormatProvider"/> and <paramref name="format"/>
        /// for formatting.
        /// </summary>
        /// <param name="format">
        ...
        public string ToString(string format, IFormatProvider fp, ICustomFormatProvider nameProvider = null, string[] nativeDigits = null)
        {
            if (fp != null)
            {
                var dtfi = DateTimeFormatInfo.GetInstance(fp);
        ...
        /// </summary>
        /// <param name="cal">
        /// The <see cref="System.Globalization.Calendar"/> to use. If null the date <paramref name="year"/>.
        /// <paramref name="month"/>.<c>1</c> is returned.
        /// </param>
        /// <param name="year">
        /// The year to use.
        /// </param>
        ...
        /// </param>
        /// <returns>
        /// The gregorian date for the first date in the month specified by <paramref name="year"/>,
        ...
        private DateTime InternalAddMonths(int months)
        {
            ...
            {
                var dt = this.Date;
                var monthsToAdd = Math.Abs(months);
                var sign = Math.Sign(months);
                var counter = 0;

                while (counter != monthsToAdd)
                {
                    dt = this._calendar.AddMonths(dt, sign);
                    counter++;
                }
            ...
            return months < 0 ? this._calendar.MinSupportedDateTime.Date : this._calendar.MaxSupportedDateTime.Date;
        }
        ...
        private string ToString(string format, DateTimeFormatInfo dtfi, ICustomFormatProvider nameProvider, string[] nativeDigits = null)
        {
            ...
            var c = nameProvider != null ? nameProvider.Calendar : dtfi.Calendar;
        ...
        /// <summary>
        /// Gets the abbreviated or full day name for the specified <see cref="DayOfWeek"/> value using, if not null, the
        /// <paramref name="nameProvider"/> or the <see cref="DateTimeFormatInfo"/> specified by
        /// <paramref name="info"/>.
        /// </summary>
        ...
        /// <summary>
        /// Gets the abbreviated or full month name for the specified <paramref name="month"/> and
        /// <paramref name="year"/> value, using the <paramref name="nameProvider"/> if not null or the
        /// <see cref="DateTimeFormatInfo"/> specified by
        /// <paramref name="info"/>.
        /// </summary>
        /// <param name="month">
