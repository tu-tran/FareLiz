namespace SkyDean.FareLiz.WinForm.Components.Controls.DatePicker.DatePicker
{
    ...
    {
        /// <summary>
        /// Event that is raised if a new date is to be set and provides the possibility to check if the date to be set is valid in a custom situation
        /// or not.
        /// </summary>
        public event EventHandler<CheckDateEventArgs> CheckDate;

        /// <summary>The selected date part.</summary>
        ...
        /// <summary>The text box for manual input.</summary>
        private readonly InputDateTextBox _inputBox;

        /// <summary>Gets the input box.</summary>
        internal InputDateTextBox InputBox
        {
            get
            {
        ...
        /// <summary>The parent date picker control.</summary>
        private readonly EnhancedDatePicker _enhancedDatePicker;

        /// <summary>Indicates that a date part is entered manually.</summary>
        private bool _inEditMode;

        /// <summary>Indicates whether the selected date is valid.</summary>
        private bool _isValidDate = true;

        /// <summary>The selected date part.</summary>
        private SelectedDatePart _selectedPart = SelectedDatePart.None;

        /// <summary>The day bounds.</summary>
        private RectangleF _dayBounds;

        /// <summary>The month bounds.</summary>
        private RectangleF _monthBounds;

        /// <summary>The year bounds.</summary>
        private RectangleF _yearBounds;

        /// <summary>The current date.</summary>
        private DateTime _currentDate;

        /// <summary>The background color for invalid dates.</summary>
        private Color _invalidDateBackColor;

        /// <summary>The text color for invalid dates.</summary>
        private Color _invalidDateForeColor;

        /// <summary>The day part index of the date string.</summary>
        private int _dayPartIndex;

        /// <summary>The month part index of the date string.</summary>
        private int _monthPartIndex;

        /// <summary>The year part index of the date string.</summary>
        ...
        /// </summary>
        /// <param name="picker">
        /// The parent <see cref="EnhancedDatePicker"/> control.
        /// </param>
        /// <exception cref="ArgumentNullException">
        /// If <paramref name="picker"/> is <c>null</c>.
        /// </exception>
        public DatePickerDateTextBox(EnhancedDatePicker picker)
        {
            ...
            this.SetStyle(
                ControlStyles.AllPaintingInWmPaint | ControlStyles.OptimizedDoubleBuffer | ControlStyles.Selectable | ControlStyles.Opaque
            ...
            this._inputBox = new InputDateTextBox(this) { Visible = false, Multiline = true, ShortcutsEnabled = false };
        ...
        /// <summary>Prevents a default instance of the <see cref="DatePickerDateTextBox" /> class from being created.</summary>
        ...
        /// <summary>Gets or sets the background color if an entered date is invalid.</summary>
        public Color InvalidBackColor
        {
            get
            {
                return this._invalidDateBackColor;
            }
        ...
        /// <summary>Gets or sets the text color if an entered date is invalid.</summary>
        public Color InvalidForeColor
        {
            get
            {
                return this._invalidDateForeColor;
            }
        ...
        /// <summary>Gets or sets the minimum visible date.</summary>
        public DateTime MinDate { get; set; }

        /// <summary>Gets or sets the maximum visible date.</summary>
        public DateTime MaxDate { get; set; }

        /// <summary>Gets the currently used culture.</summary>
        public CultureInfo Culture
        {
            get
            {
                return this._enhancedDatePicker.Culture;
            }
        }
        ...
        public override bool Focused
        {
            get
            {
                return base.Focused || this._inputBox.Focused;
            }
        }
        ...
        protected override bool ProcessDialogKey(Keys keyData)
        {
            ...
            var cal = this._enhancedDatePicker.PickerCalendar.CultureCalendar;

            var dt = new MonthCalendarDate(cal, this._currentDate);
            ...
            if (keyData == Keys.Up || keyData == Keys.Down)
            {
                ...
                switch (this._selectedPart)
                {
                    case SelectedDatePart.Day:
                        {
                            var day = dt.Day + (up ? 1 : -1);

                            var daysInMonth = DateMethods.GetDaysInMonth(dt);
                    ...
                    case SelectedDatePart.Month:
                        {
                            ...
                            var month = dt.Month + (up ? 1 : -1);

                            var monthsInYear = cal.GetMonthsInYear(dt.Year);
                            ...
                            var newDate = new DateTime(dt.Year, month, 1, cal);
                    ...
                    case SelectedDatePart.Year:
                        {
                            var year = dt.Year + (up ? 1 : -1);
                            var minYear = cal.GetYear(this.MinDate);
                            var maxYear = cal.GetYear(this.MaxDate);

                            year = Math.Max(minYear, Math.Min(year, maxYear));

                            var yearDiff = year - dt.Year;
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            using (var format = new StringFormat(StringFormatFlags.LineLimit | StringFormatFlags.NoClip | StringFormatFlags.NoWrap))
            {
                ...
                {
                    var cal = this._enhancedDatePicker.PickerCalendar;
                    var provider = cal.FormatProvider;

                    var date = new MonthCalendarDate(cal.CultureCalendar, this._currentDate);

                    var parser = new DatePatternParser(provider.LongDatePattern, provider);

                    var dateString = parser.ParsePattern(
                        date, 
                    ...
                    var rangeList = new List<CharacterRange>();

                    var dayIndex = parser.DayIndex;
                    var monthIndex = parser.MonthIndex;
                    var yearIndex = parser.YearIndex;
                    ...
                    var layoutRect = this.ClientRectangle;

                    e.Graphics.DrawString(dateString, this.Font, foreBrush, layoutRect, format);

                    var dateRegions = e.Graphics.MeasureCharacterRanges(dateString, this.Font, layoutRect, format);
        ...
        /// <summary>
        /// Releases the unmanaged resources used by the System.Windows.Forms.Control and its child controls and optionally releases the managed
        /// resources.
        /// </summary>
        ...
        /// <summary>
        /// Determines if the specified <paramref name="day"/> is a valid day in regard to the <paramref name="date"/>
        /// value.
        /// </summary>
        /// <param name="day">
        /// The day value.
        /// </param>
        /// <param name="date">
        /// The year and month value.
        /// </param>
        /// <param name="cal">
        /// The calendar to use.
        /// </param>
        /// <returns>
        /// true if it's a valid day; false otherwise.
        /// </returns>
        ...
        /// </param>
        /// <returns>
        /// true if it's a valid month value; false otherwise.
        /// </returns>
        ...
        /// </param>
        /// <param name="era">
        /// The era the year belongs to.
        /// </param>
        /// <returns>
        /// true if it's a valid year value; false otherwise.
        /// </returns>
        private static bool IsValidYear(int year, Calendar cal, int era)
        {
            var minYear = cal.GetYear(cal.MinSupportedDateTime.Date);
            var maxYear = cal.GetYear(cal.MaxSupportedDateTime.Date);

            if (cal.Eras.Length > 1)
            {
                DateTime? minDate = null, maxDate = null;

                var date = cal.MinSupportedDateTime;

                while (date < cal.MaxSupportedDateTime.Date)
                {
                    var e = cal.GetEra(date);
        ...
        /// </summary>
        /// <param name="left">
        /// true for selecting the next left date part; false for the next right date part.
        /// </param>
        ...
        /// The index of the currently selected date part.
        /// </param>
        /// <param name="left">
        /// The moving direction.
        /// </param>
        /// <returns>
        /// The next selected date part.
        /// </returns>
        private SelectedDatePart GetNextSelectedPart(int currentIndex, bool left)
        {
            var newIndex = currentIndex + (left ? -1 : 1);
        ...
        private void SetNewDate(DateTime date)
        {
            ...
            {
                var checkEventArgs = new CheckDateEventArgs(date, true);
        ...
        private void InputBoxFinishedEditing(object sender, EventArgs e)
        {
            ...
            var inputStr = this._inputBox.GetCurrentText();

            var containsSeparator = inputStr.Contains(this._enhancedDatePicker.FormatProvider.DateSeparator);
            ...
            if (match.Success && !string.IsNullOrEmpty(dayString) && !string.IsNullOrEmpty(monthString) && !string.IsNullOrEmpty(yearString))
            {
                ...
                {
                    var date = new DateTime(year, int.Parse(monthString), 1, cal);
