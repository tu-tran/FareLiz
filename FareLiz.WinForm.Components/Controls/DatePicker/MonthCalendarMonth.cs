namespace SkyDean.FareLiz.WinForm.Components.Controls.DatePicker
{
    ...
    /// <summary>Class that represents a single month in the <see cref="Calendar" />.</summary>
    public class MonthCalendarMonth
    {
        ...
        public MonthCalendarMonth(EnhancedMonthCalendar enhancedMonthCal, DateTime date)
        {
            ...
            var dt = new MonthCalendarDate(enhancedMonthCal.CultureCalendar, date).FirstOfMonth.GetFirstDayInWeek(enhancedMonthCal.FormatProvider);

            var dayList = new List<MonthCalendarDay>();
            var weekList = new List<MonthCalendarWeek>();
            ...
            var d = dayAdjust != 0 ? 8 - dayAdjust : 0;

            for (var i = dayAdjust; i < 42 + dayAdjust; i++, dt = dt.AddDays(1))
            {
                var day = new MonthCalendarDay(this, dt.Date);
                ...
                {
                    var weekEnd = dt.GetEndDateOfWeek(enhancedMonthCal.FormatProvider).Date;

                    var weekNumEnd = DateMethods.GetWeekOfYear(enhancedMonthCal.Culture, enhancedMonthCal.CultureCalendar, weekEnd);
        ...
        /// <summary>The _location of the month.</summary>
        private Point _location;

        /// <summary>The first visible date.</summary>
        private readonly DateTime _firstVisibleDate = DateTime.MinValue;

        /// <summary>The last visible date.</summary>
        ...
        /// <summary>Gets or sets the index of the month.</summary>
        public int Index { get; set; }

        /// <summary>Gets the bounds of this instance.</summary>
        public Rectangle Bounds
        {
            get
            {
                return new Rectangle(this._location, this.Size);
            }
        }

        /// <summary>Gets the <see cref="Calendar" /> which hosts the month.</summary>
        public EnhancedMonthCalendar Calendar { get; private set; }

        /// <summary>Gets the date which specifies the displayed month and year.</summary>
        public DateTime Date { get; private set; }

        /// <summary>Gets or sets the _location of the this month.</summary>
        ...
        /// <summary>Gets the <see cref="MonthCalendarDay" /> instances that are being hosted.</summary>
        public MonthCalendarDay[] Days { get; private set; }

        /// <summary>Gets or sets the bounds of the days.</summary>
        public Rectangle DayNamesBounds { get; set; }

        /// <summary>Gets or sets the weeks displayed.</summary>
        public MonthCalendarWeek[] Weeks { get; set; }

        /// <summary>Gets or sets the title bounds where the month and year is displayed.</summary>
        public Rectangle TitleBounds { get; set; }

        /// <summary>Gets or sets the bounds for the month in the header.</summary>
        public Rectangle TitleMonthBounds { get; set; }

        /// <summary>Gets or sets the bounds for the year in the header.</summary>
        public Rectangle TitleYearBounds { get; set; }

        /// <summary>Gets or sets the week bounds.</summary>
        public Rectangle WeekBounds { get; set; }

        /// <summary>Gets or sets the bounds in which the days are drawn.</summary>
        public Rectangle MonthBounds { get; set; }

        /// <summary>Gets the size of this instance of <see cref="MonthCalendarMonth" />.</summary>
        public Size Size
        {
            get
            {
                return this.Calendar.MonthSize;
            }
        }

        /// <summary>Gets the first visible date.</summary>
        public DateTime FirstVisibleDate
        {
            get
            {
                return this._firstVisibleDate;
            }
        }

        /// <summary>Gets the last visible date.</summary>
        public DateTime LastVisibleDate
        {
            get
            {
                return this._lastVisibleDate;
            }
        }

        /// <summary>Gets a value indicating whether to draw the left button.</summary>
        internal bool DrawLeftButton
        {
            get
            {
                var result = (this.Date > this.Calendar.MinDate)
                             && (this.Calendar.UseRTL ? this.Index == this.Calendar.CalendarDimensions.Width - 1 : this.Index == 0);
                return result;
            }
        }

        /// <summary>Gets a value indicating whether to draw the right button.</summary>
        internal bool DrawRightButton
        {
            get
            {
                var result = (this.Date < this.Calendar.MaxDate)
                             && (this.Calendar.UseRTL ? this.Index == 0 : this.Index == this.Calendar.CalendarDimensions.Width - 1);
                return result;
            }
        }
        ...
        /// </summary>
        /// <param name="loc">
        /// The position to perform the hit test for.
        /// </param>
        /// <returns>
        /// An <see cref="MonthCalendarHitTest"/> that contains the hit test data.
        /// </returns>
        public MonthCalendarHitTest HitTest(Point loc)
        {
            if (this.TitleBounds.Contains(loc))
            {
                var dt = this.Calendar.CultureCalendar.GetEra(this.Date) != this.Calendar.CultureCalendar.GetEra(this._firstVisibleDate)
            ...
            {
                var dayWidth = this.Calendar.DaySize.Width;
                var dayNameBounds = this.DayNamesBounds;
        ...
        /// </param>
        /// <returns>
        /// true if the specified date is visible; otherwise false.
        /// </returns>
        ...
        private void CalculateProportions(Point loc)
        {
            ...
            var useRTL = this.Calendar.UseRTL;
            var adjustX = this.Calendar.WeekNumberSize.Width;
            var dayWidth = this.Calendar.DaySize.Width;
            var dayHeight = this.Calendar.DaySize.Height;
            var weekRectAdjust = 0;
            ...
            var weekNumberRect = new Rectangle(
            ...
            var weekBounds = weekNumberRect;

            // calculate month body bounds
            var monthRect = new Rectangle(loc.X + adjustX, loc.Y + this.TitleBounds.Height + this.DayNamesBounds.Height, dayWidth * 7, dayHeight * 6);

            // get start position at where to draw
            var startX = monthRect.X;
            adjustX = dayWidth;
            ...
            {
                var currentDayOfWeek = this.Calendar.FormatProvider.FirstDayOfWeek;
                var dayOfWeek = new MonthCalendarDate(this.Calendar.CultureCalendar, this.Days[0].Date).DayOfWeek;
                ...
                while (currentDayOfWeek != dayOfWeek)
                {
                    ...
                    var nextDay = (int)currentDayOfWeek + 1;
            ...
            for (var i = 0; i < this.Days.Length; i++, j++)
            {
                ...
                if (i % 7 == 0)
                {
                    ...
                    var visible = this.Days[i].Visible || this.Days[Math.Min(i + 6, this.Days.Length - 1)].Visible;
