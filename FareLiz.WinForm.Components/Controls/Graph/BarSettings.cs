...
namespace SkyDean.FareLiz.WinForm.Components.Controls.Graph
{
	...
	#region Fields
		...
		/// increasing value. The value is of the enumeration type <see cref="BarBase"/>.
		...
		/// graphs will be displayed. See the <see cref="BarType"/> enum
...
//			else if ( minStep <= 0 || minStep < 0.001 * range || minStep > range )
		...
		/// (the base <see cref="Axis"/> is assigned by the <see cref="BarSettings.Base"/>
		/// property).
		/// </remarks>
	...
	#region Defaults
		...
		public struct Default
		{
			...
			/// or side-by-side in a "cluster" format.  See the <see cref="BarType"/>
