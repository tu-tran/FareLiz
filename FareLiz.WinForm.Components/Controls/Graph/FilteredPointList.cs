...
namespace SkyDean.FareLiz.WinForm.Components.Controls.Graph
{
    ...
    /// <summary>
    /// An example of an <see cref="IPointList" /> implementation that stores large datasets, and selectively filters the output data depending on the
    /// displayed range.
    /// </summary>
    /// <remarks>
    /// This class will refilter the data points each time <see cref="SetBounds" /> is called.  The data are filtered down to <see cref="MaxPts" /> points,
    /// within the data bounds of a minimum and maximum data range.  The data are filtered by simply skipping points to achieve the desired total number of
    /// points.  Input arrays are assumed to be monotonically increasing in X, and evenly spaced in X.
    /// </remarks>
    ...
    public class FilteredPointList : IPointList
    {
        ...
        /// <summary>
        /// Set the data bounds to the specified minimum, maximum, and point count.  Use values of min=double.MinValue and max=double.MaxValue to get the full
        /// range of data.  Use maxPts=-1 to not limit the number of points.  Call this method anytime the zoom range is changed.
        /// </summary>
        /// <param name="min">
        ...
        /// </param>
        /// <param name="maxPts">
        /// The maximum number of points allowed to be output by the filter
        /// </param>
        ...
        /// <summary>Instance of an array of x values</summary>
        private double[] _x;
		...
		*/
        ...
        /// <summary>This is the maximum number of points that you want to see in the filtered dataset</summary>
        private int _maxPts = -1;

        /// <summary>The index of the xMinBound above</summary>
        private int _minBoundIndex = -1;

        /// <summary>The index of the xMaxBound above</summary>
            ...
            get
            {
                ...
                if (this._minBoundIndex >= 0 && this._maxBoundIndex >= 0 && this._maxPts >= 0)
                {
                    ...
                    if (nPts > this._maxPts)
                    {
                        ...
                        index = this._minBoundIndex + (int)(index * (double)nPts / this._maxPts);
                    }
                    else
                    {
        ...
        /// <summary>Returns the number of points according to the current RequestState of the filter.</summary>
        public int Count
        {
            get
            {
        ...
        /// <summary>Gets the desired number of filtered points to output.  You can set this value by calling <see cref="SetBounds" />.</summary>
        public int MaxPts
        {
            get
            {
