...
namespace SkyDean.FareLiz.WinForm.Components.Controls.Graph
{
    ...
    /// <summary>
    /// Encapsulates an "High-Low" Bar curve type that displays a bar in which both the bottom and the top of the bar are set by data valuesfrom the
    /// <see cref="PointPair" /> struct.
    /// </summary>
    /// <remarks>
    /// The <see cref="HiLowBarItem" /> type is intended for displaying bars that cover a band of data, such as a confidence interval, "waterfall" chart,
    /// etc.  The position of each bar is set according to the <see cref="PointPair" /> values.  The independent axis is assigned with
    /// <see cref="BarSettings.Base" />, and is a
    /// <see cref="BarBase" /> enum type.  If <see cref="BarSettings.Base" />
    /// is set to <see cref="BarBase.Y" /> or <see cref="BarBase.Y2" />, then the bars will actually be horizontal, since the X axis becomes the value axis
    /// and the Y or Y2 axis becomes the independent axis.
    /// </remarks>
    ...
    public class HiLowBarItem : BarItem, ICloneable, ISerializable
    {
        ...
        /// </param>
        /// <param name="color">
        /// A <see cref="Color"/> value that will be applied to the <see cref="Bar.Fill"/> and <see cref="Bar.Border"/> properties.
        /// </param>
