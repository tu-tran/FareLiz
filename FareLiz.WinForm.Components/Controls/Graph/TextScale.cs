...
namespace SkyDean.FareLiz.WinForm.Components.Controls.Graph
{
    ...
    /// <summary>The TextScale class inherits from the <see cref="Scale" /> class, and implements the features specific to <see cref="AxisType.Text" />.</summary>
    /// <remarks>
    /// TextScale is an ordinal axis with user-defined text labels.  An ordinal axis means that all data points are evenly spaced at integral values, and the
    /// actual coordinate values for points corresponding to that axis are ignored.  That is, if the X axis is an ordinal type, then all X values associated
    /// with the curves are ignored.
    /// </remarks>
    ...
    internal class TextScale : Scale, ISerializable
    {
        ...
        /// </summary>
        /// <remarks>
        /// This method only applies to <see cref="AxisType.Text"/> type axes, and it is called by the general <see cref="PickScale"/> method.  This is an
        /// ordinal type, such that the labeled values start at 1.0 and increment by 1.0 for each successive label.  The maximum number of labels on the graph is
        /// determined by <see cref="Scale.Default.MaxTextLabels"/>.  If necessary, this method will set the <see cref="Scale.MajorStep"/> value to greater
        /// than 1.0 in order to keep the total labels displayed below <see cref="Scale.Default.MaxTextLabels"/>.  For example, a
        /// <see cref="Scale.MajorStep"/> size of 2.0 would only display every other label on the axis.  The <see cref="Scale.MajorStep"/> value calculated by
        /// this routine is always an integral value.  This method honors the <see cref="Scale.MinAuto"/>, <see cref="Scale.MaxAuto"/>, and
        ...
        /// setting is explicitly honored, and the remaining autorange settings (if any) will be calculated to accomodate the non-autoranged values.
        /// <para>
        /// On Exit:
        /// </para>
        /// <para>
