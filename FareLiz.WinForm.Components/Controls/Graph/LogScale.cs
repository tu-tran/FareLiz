...
namespace SkyDean.FareLiz.WinForm.Components.Controls.Graph
{
    ...
    /// <summary>The LogScale class inherits from the <see cref="Scale" /> class, and implements the features specific to <see cref="AxisType.Log" />.</summary>
    /// <remarks>LogScale is a non-linear axis in which the values are scaled using the base 10
    ...
    internal class LogScale : Scale, ISerializable
    {
        ...
        /// <remarks>
        /// The set property is specifically adapted for <see cref="AxisType.Log" /> scales, in that it automatically limits the setting to values greater than
        /// zero.
        /// </remarks>
        ...
        {
            return baseVal + tic * this.CyclesPerStep;
        ...
        internal override double CalcMinorTicValue(double baseVal, int iTic)
        {
            double[] dLogVal =
                {
                    0, 0.301029995663981, 0.477121254719662, 0.602059991327962, 0.698970004336019, 0.778151250383644, 0.845098040014257, 
        ...
        internal override double CalcBaseTic()
        {
            ...
            return Math.Ceiling(SafeLog(this._min) - 0.00000001);
        }
        ...
        internal override int CalcNumTics()
        {
            ...
            nTics = (int)((SafeLog(this._max) - SafeLog(this._min)) / this.CyclesPerStep) + 1;
        ...
        /// </summary>
        /// <remarks>
        /// This method only applies to <see cref="AxisType.Log"/> type axes, and it is called by the general <see cref="PickScale"/> method.  The scale range
        /// is chosen based always on powers of 10 (full log cycles).  This method honors the <see cref="Scale.MinAuto"/>, <see cref="Scale.MaxAuto"/>, and
        /// <see cref="Scale.MajorStepAuto"/> autorange settings. In the event that any of the autorange settings are false, the corresponding
        /// <see cref="Scale.Min"/>, <see cref="Scale.Max"/>, or <see cref="Scale.MajorStep"/>
        /// setting is explicitly honored, and the remaining autorange settings (if any) will be calculated to accomodate the non-autoranged values.  For log
        /// axes, the MinorStep value is not used.
        /// <para>
        /// On Exit:
        /// </para>
        /// <para>
        ...
        public override void PickScale(GraphPane pane, Graphics g, float scaleFactor)
        {
            ...
            if (this._minAuto)
            {
                this._min = Math.Pow(10.0, Math.Floor(Math.Log10(this._min)));
            }

            if (this._maxAuto)
            {
                this._max = Math.Pow(10.0, Math.Ceiling(Math.Log10(this._max)));
            }
        }
