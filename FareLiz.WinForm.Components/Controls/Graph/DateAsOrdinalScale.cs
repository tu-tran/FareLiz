...
namespace SkyDean.FareLiz.WinForm.Components.Controls.Graph
{
    ...
    /// <summary>
    /// The DateAsOrdinalScale class inherits from the <see cref="Scale" /> class, and implements the features specific to
    /// <see cref="AxisType.DateAsOrdinal" />.
    /// </summary>
    /// <remarks>
    /// DateAsOrdinalScale is an ordinal axis that will have labels formatted with dates from the actual data values of the first <see cref="CurveItem" /> in
    /// the <see cref="CurveList" />. Although the tics are labeled with real data values, the actual points will be evenly-spaced in spite of the data
    /// values.  For example, if the X values of the first curve are 1, 5, and 100, then the tic labels will show 1, 5, and 100, but they will be equal
    ...
    internal class DateAsOrdinalScale : Scale, ISerializable
    {
        ...
        /// </param>
        /// <param name="owner">
        /// The <see cref="Axis"/> object that will own the new instance of <see cref="DateAsOrdinalScale"/>
        /// </param>
        ...
        /// <see cref="AxisType.DateAsOrdinal" />.</summary>
        ...
        /// <remarks>
        /// The set property is specifically adapted for <see cref="AxisType.DateAsOrdinal" /> scales, in that it automatically limits the value to the range of
        /// valid dates for the
        ...
        /// <summary>
        /// Select a reasonable ordinal axis scale given a range of data values, with the expectation that dates will be displayed.
        /// </summary>
        /// <remarks>
        /// This method only applies to <see cref="AxisType.DateAsOrdinal"/> type axes, and it is called by the general <see cref="PickScale"/> method.  For
        /// this type, the first curve is the "master", which contains the dates to be applied.
        /// <para>
        /// On Exit:
        /// </para>
        /// <para>
        ...
        internal void SetDateFormat(GraphPane pane)
        {
            if (this._formatAuto)
            {
                ...
                if (pane.CurveList.Count > 0 && pane.CurveList[0].Points.Count > 1)
                {
                    ...
                    if (val1 != PointPairBase.Missing && val2 != PointPairBase.Missing && !double.IsNaN(val1) && !double.IsNaN(val2)
                        && !double.IsInfinity(val1) && !double.IsInfinity(val2) && Math.Abs(val2 - val1) > 1e-10)
                    {
