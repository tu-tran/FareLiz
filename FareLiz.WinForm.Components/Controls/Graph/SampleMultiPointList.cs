...
namespace SkyDean.FareLiz.WinForm.Components.Controls.Graph
{
    ...
    {
        /// <summary>The time (seconds) at which these data are measured</summary>
        Time, 

        /// <summary>The distance traveled, meters</summary>
        Distance, 

        /// <summary>The instantaneous velocity, meters per second</summary>
        Velocity, 

        /// <summary>The instantaneous acceleration, meters per second squared</summary>
        Acceleration
    };

    /// <summary>
    /// Sample data structure containing a variety of data values, in this case the values are related in that they correspond to the same time
    /// value.
    /// </summary>
    ...
    /// </summary>
    /// <remarks>
    /// This particular class efficiently implements the data storage so that the class can be cloned without duplicating the data points.  For example, you
    /// can create a <see cref="SampleMultiPointList" />, populate it with values, and set
    /// <see cref="XData" /> = <see cref="PerfDataType.Time" /> and
    /// <see cref="YData" /> = <see cref="PerfDataType.Distance" />. You can then clone this <see cref="SampleMultiPointList" /> to a new one, and set
    /// <see cref="YData" /> = <see cref="PerfDataType.Velocity" />. Each of these <see cref="SampleMultiPointList" />'s can then be used as an
    /// <see cref="GraphPane.AddCurve(string,IPointList,Color)" /> argument, thereby plotting a distance vs time curve and a velocity vs time curve.  There
    ...
    public class SampleMultiPointList : IPointList
    {
        ...
        /// copies the reference to this <see cref="ArrayList" />, but does not actually duplicate the data.
        /// </summary>
        private ArrayList DataCollection;

        /// <summary>Determines what X data will be returned by the indexer of this list.</summary>
        public PerfDataType XData;

        /// <summary>Determines what Y data will be returned by the indexer of this list.</summary>
        public PerfDataType YData;

        /// <summary>
        /// Initializes a new instance of the <see cref="SampleMultiPointList"/> class. Default constructor
        /// </summary>
        ...
        /// The Copy Constructor.  This method does NOT duplicate the data, it merely makes another "Window" into the same collection.  You can make multiple
        /// copies and set the <see cref="XData"/> and/or <see cref="YData"/> properties to different values to plot different data, while maintaining only one
        ...
        /// <summary>
        /// Indexer to access the data.  This gets the appropriate data and converts to the <see cref="PointPair"/> struct that is compatible with ZedGraph.
        /// The actual data returned depends on the values of <see cref="XData"/> and
        /// <see cref="YData"/>.
        /// </summary>
        /// <param name="index">
        ...
        /// <summary>Gets the number of data points in the collection</summary>
        public int Count
        {
            get
            {
        ...
        /// <summary>
        /// Remove the <see cref="PerformanceData"/> struct from the list at the specified ordinal location.
        /// </summary>
        /// <param name="index">
