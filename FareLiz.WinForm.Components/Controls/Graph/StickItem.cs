...
namespace SkyDean.FareLiz.WinForm.Components.Controls.Graph
{
    ...
    /// <summary>Encapsulates a curve type that is displayed as a series of vertical "sticks", one at each defined point.</summary>
    /// <remarks>
    /// The sticks run from the zero value of the Y axis, to the Y point defined in each
    /// <see cref="PointPair" /> of the <see cref="IPointList" /> (see <see cref="CurveItem.Points" />). The properties of the sticks are defined in the
    /// <see cref="Line" /> property. Normally, the <see cref="Symbol" /> is not visible.  However, if you manually enable the
    /// <see cref="Symbol" /> using the <see cref="Symbol.IsVisible" /> property, the symbols will be drawn at the "Z" value from each
    /// <see cref="PointPair" /> (see
