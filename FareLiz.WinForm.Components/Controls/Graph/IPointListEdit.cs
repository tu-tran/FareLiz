...
{
    /// <summary>An interface to a collection class containing data that define the set of points to be displayed on the curve.</summary>
    /// <remarks>
    /// This interface is designed to allow customized data abstraction.  The default data collection class is <see cref="PointPairList" />, however, you can
    /// define your own data collection class using the <see cref="IPointList" /> interface.  This
    /// <see cref="IPointListEdit" /> interface adds the ability to remove and add points to the list, and so is used by the <see cref="CurveItem" /> class
    /// for the
    ...
    public interface IPointListEdit : IPointList
    {
        ...
        /// </summary>
        /// <remarks>
        /// This is the standard interface that ZedGraph uses to access the data.  Although you must pass a <see cref="PointPair"/> here, your internal data
        /// storage format can be anything.
        /// </remarks>
        /// <param name="index">
        ...
        /// <summary>Clears all data points from the list.  After calling this method,
        /// <see cref="IPointList.Count" /> will be zero.</summary>
        void Clear();
    }
}