...
namespace SkyDean.FareLiz.WinForm.Components.Controls.Graph
{
    ...
    /// <summary>
    /// A class that provides a rolling list of <see cref="PointPair" /> objects. This is essentially a first-in-first-out (FIFO) queue with a fixed capacity
    /// which allows 'rolling' (or oscilloscope like) graphs to be be animated without having the overhead of an ever-growing ArrayList. The queue is
    /// constructed with a fixed capacity and new points can be enqueued. When the capacity is reached the oldest (first in) PointPair is overwritten.
    /// However, when accessing via <see cref="IPointList" />, the <see cref="PointPair" /> objects are seen in the order in which they were enqeued.
    ...
    public class RollingPointPairList : IPointList, ISerializable, IPointListEdit
    {
        ...
        /// <summary>An array of PointPair objects that acts as the underlying buffer.</summary>
        protected PointPair[] _mBuffer;

        /// <summary>The index of the previously enqueued item. -1 if buffer is empty.</summary>
        protected int _headIdx;

        /// <summary>The index of the next item to be dequeued. -1 if buffer is empty.</summary>
        ...
        /// </summary>
        /// <param name="capacity">
        /// Number of elements in the rolling list.  This number cannot be changed once the RollingPointPairList is constructed.
        /// </param>
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="RollingPointPairList"/> class. 
        /// Constructs an empty buffer with the specified capacity.  Pre-allocates space for all PointPair's in the list if <paramref name="preLoad"/>
        /// is true.
        /// </summary>
        /// <param name="capacity">
        /// Number of elements in the rolling list.  This number cannot be changed once the RollingPointPairList is constructed.
        /// </param>
        /// <param name="preLoad">
        /// true to pre-allocate all PointPair instances in the list, false otherwise.  Note that in order to be memory efficient, the
        /// <see cref="Add(double,double,double)"/> method should be used to add data.  Avoid the <see cref="Add(PointPair)"/> method.
        /// </param>
        ...
        /// <see cref="IPointList"/>. The <see cref="Capacity"/> is set to the length of the provided list.
        /// </summary>
        /// <param name="rhs">
        ...
        /// <summary>Gets the capacity of the rolling buffer.</summary>
        public int Capacity
        {
            get
            {
        ...
        /// <summary>Gets the count of items within the rolling buffer. Note that this may be less than the capacity.</summary>
        public int Count
        {
            get
            {
        ...
        /// <summary>Gets a bolean that indicates if the buffer is empty. Alternatively you can test Count==0.</summary>
        public bool IsEmpty
        {
            get
            {
        ...
        /// </param>
        /// <remarks>
        /// Index must be within the current size of the buffer, e.g., the set method will not expand the buffer even if <see cref="Capacity"/> is
        /// available
        /// </remarks>
        /// <returns>
        ...
        /// <summary>Clear the buffer of all <see cref="PointPair" /> objects. Note that the <see cref="Capacity" /> remains unchanged.</summary>
        public void Clear()
        {
            this._headIdx = this._tailIdx = -1;
        }
        ...
        /// <summary>
        /// Calculate that the next index in the buffer that should receive a new data point. Note that this method actually advances the buffer, so a datapoint
        /// should be added at _mBuffer[_headIdx].
        /// </summary>
        ...
        /// A reference to the <see cref="IPointList"/> object to be added
        /// </param>
        ...
        /// <summary>Remove an old item from the tail of the queue.</summary>
        /// <returns>
        /// The removed item. Throws an <see cref="InvalidOperationException" />
        /// if the buffer was empty. Check the buffer's length (<see cref="Count" />) or the <see cref="IsEmpty" />
        ...
        /// </summary>
        /// <remarks>
        /// All items in the queue that lie after <paramref name="index"/> will be shifted back by one, and the queue will be one item shorter.
        /// </remarks>
        /// <param name="index">
        /// The ordinal position of the item to be removed. Throws an <see cref="ArgumentOutOfRangeException"/> if index is less than zero or greater than or
        ...
        /// </summary>
        /// <remarks>
        /// All items in the queue that lie after <paramref name="index"/> will be shifted back, and the queue will be <paramref name="count"/> items
        /// shorter.
        /// </remarks>
        /// <param name="index">
        /// The ordinal position of the item to be removed. Throws an <see cref="ArgumentOutOfRangeException"/> if index is less than zero or greater than or
        ...
        /// <see cref="ArgumentOutOfRangeException"/> if <paramref name="count"/> is less than zero or greater than the total available items in the queue
        /// </param>
        ...
        /// <summary>Pop an item off the head of the queue.</summary>
        ...
        /// <summary>Peek at the <see cref="PointPair" /> item at the head of the queue.</summary>
        /// <returns>The <see cref="PointPair" /> item at the head of the queue. Throws an <see cref="InvalidOperationException" /> if the buffer was empty.</returns>
        ...
        /// method, since it does not require that a new PointPair instance be provided. If the buffer already contains a <see cref="PointPair"/> at the head
        /// position, then the x, y, z, and tag values will be copied into the existing PointPair. Otherwise, a new PointPair instance must be created. In this
        /// way, each PointPair position in the rolling list will only be allocated one time. To truly be memory efficient, the <see cref="Remove"/>,
        /// <see cref="RemoveAt"/>, and <see cref="Pop"/> methods should be avoided.  Also, the <paramref name="tag"/> property for this method should be
        /// null, since it is a reference type.
        /// </remarks>
        /// <param name="x">
        /// The X value
        /// </param>
        ...
        /// position, then the x, y, z, and tag values will be copied into the existing PointPair. Otherwise, a new PointPair instance must be created. In this
        /// way, each PointPair position in the rolling list will only be allocated one time. To truly be memory efficient, the <see cref="Remove"/>,
        /// <see cref="RemoveAt"/>, and <see cref="Pop"/> methods should be avoided.
        /// </remarks>
        /// <param name="x">
        /// The X value
        /// </param>
        ...
        {
            this.Add(x, y, PointPairBase.Missing, null);
        }
        ...
        {
            this.Add(x, y, PointPairBase.Missing, tag);
        }
        ...
        /// from two arrays of type double. If either array is null, then a set of ordinal values is automatically generated in its place (see
        /// <see cref="AxisType.Ordinal"/>). If the arrays are of different size, then the larger array prevails and the smaller array is padded with
