...
namespace SkyDean.FareLiz.WinForm.Components.Controls.Graph
{
    ...
    partial class ZedGraphControl
    {
        ...
        /// <summary>Public enumeration that specifies the type of object present at the Context Menu's mouse location</summary>
        public enum ContextMenuObjectState
        {
            /// <summary>The object is an Inactive Curve Item at the Context Menu's mouse position</summary>
            InactiveSelection, 

            /// <summary>The object is an active Curve Item at the Context Menu's mouse position</summary>
            ActiveSelection, 

            /// <summary>There is no selectable object present at the Context Menu's mouse position</summary>
            Background
        }

        // Revision: JCarpenter 10/06
        /// <summary>
        /// Find the object currently under the mouse cursor, and return its RequestState.
        /// </summary>
        /// <returns>
        ...
        private void contextMenuStrip1_Opening(object sender, CancelEventArgs e)
        {
            ...
            if (this._masterPane != null && menuStrip != null)
            {
                ...
                this._menuClickPt = this.PointToClient(MousePosition);
                ...
                if (this._isShowContextMenu)
                {
                    ...
                    item.Tag = item.Name = "copy";
                    item.Text = this._resourceManager.GetString("copy");
                    item.Click += this.MenuClick_Copy;
                    menuStrip.Items.Add(item);

                    item = new ToolStripMenuItem();
                    item.Tag = item.Name = "save_as";
                    item.Text = this._resourceManager.GetString("save_as");
                    item.Click += this.MenuClick_SaveAs;
                    menuStrip.Items.Add(item);

                    item = new ToolStripMenuItem();
                    item.Tag = item.Name = "page_setup";
                    item.Text = this._resourceManager.GetString("page_setup");
                    item.Click += this.MenuClick_PageSetup;
                    menuStrip.Items.Add(item);

                    item = new ToolStripMenuItem();
                    item.Tag = item.Name = "print";
                    item.Text = this._resourceManager.GetString("print");
                    item.Click += this.MenuClick_Print;
                    menuStrip.Items.Add(item);

                    item = new ToolStripMenuItem();
                    item.Tag = item.Name = "show_val";
                    item.Text = this._resourceManager.GetString("show_val");
                    item.Click += this.MenuClick_ShowValues;
                    item.Checked = this.IsShowPointValues;
                    menuStrip.Items.Add(item);

                    item = new ToolStripMenuItem();
                    item.Tag = item.Name = "unzoom";
                    ...
                    item.Text = menuStr;
                    item.Click += this.MenuClick_ZoomOut;
                    if (pane == null || pane.ZoomStack.IsEmpty)
                    {
                        item.Enabled = false;
                    }
                    ...
                    item = new ToolStripMenuItem();
                    item.Tag = item.Name = "undo_all";
                    menuStr = this._resourceManager.GetString("undo_all");
                    item.Text = menuStr;
                    item.Click += this.MenuClick_ZoomOutAll;
                    if (pane == null || pane.ZoomStack.IsEmpty)
                    {
                        item.Enabled = false;
                    }
                    ...
                    item = new ToolStripMenuItem();
                    item.Tag = item.Name = "set_default";
                    menuStr = this._resourceManager.GetString("set_default");
                    item.Text = menuStr;
                    item.Click += this.MenuClick_RestoreScale;
                    if (pane == null)
                    {
                        item.Enabled = false;
                    }
        ...
        /// boolean value that determines whether or not a prompt will be displayed.  true to show a message of "Image Copied to
        /// ClipBoard".
        /// </param>
        public void Copy(bool isShowMessage)
        {
            if (this._masterPane != null)
            {
                ...
                Thread ct = new Thread(this.ClipboardCopyThread);
        ...
        /// <summary>A threaded version of the copy method to avoid crash with MTA</summary>
        ...
        /// <summary>
        /// Setup for creation of a new image, applying appropriate anti-alias properties and returning the resultant image file
        /// </summary>
        /// <returns>
        /// The <see cref="Image"/>.
        /// </returns>
        ...
        /// </summary>
        /// <remarks>
        /// This version is similar to the regular <see cref="Copy"/> method, except that it will place an Emf image (vector) on the ClipBoard instead of the
        /// regular bitmap.
        /// </remarks>
        /// <param name="isShowMessage">
        /// boolean value that determines whether or not a prompt will be displayed.  true to show a message of "Image Copied to
        ...
        public void CopyEmf(bool isShowMessage)
        {
            if (this._masterPane != null)
            {
                // Threaded copy mode to avoid crash with MTA
                // Contributed by Dave Moor
                Thread ct = new Thread(this.ClipboardCopyThreadEmf);
        ...
        /// <summary>
        /// Handler for the "Save Image As" context menu item.  Copies the current image to the selected file in either the Emf (vector), or a variety of Bitmap
        /// formats.
        /// </summary>
        /// <remarks>
        /// Note that <see cref="SaveAsBitmap" /> and <see cref="SaveAsEmf" /> methods are provided which allow for Bitmap-only or Emf-only handling of the "Save
        ...
        /// <summary>
        /// Copies the current image to the selected file in Emf (vector), or a variety of Bitmap formats.
        /// </summary>
        ...
        public string SaveAs(string DefaultFileName)
        {
            if (this._masterPane != null)
            {
                this._saveFileDialog.Filter = "Emf Format (*.emf)|*.emf|" + "PNG Format (*.png)|*.png|" + "Gif Format (*.gif)|*.gif|"
                                              + "Jpeg Format (*.jpg)|*.jpg|" + "Tiff Format (*.tif)|*.tif|" + "Bmp Format (*.bmp)|*.bmp";
        ...
        /// <summary>Handler for the "Save Image As" context menu item.  Copies the current image to the selected Bitmap file.</summary>
        /// <remarks>Note that this handler saves as a bitmap only.  The default handler is
        /// <see cref="SaveAs()" />, which allows for Bitmap or EMF formats</remarks>
        public void SaveAsBitmap()
        {
            if (this._masterPane != null)
            {
                this._saveFileDialog.Filter = "PNG Format (*.png)|*.png|" + "Gif Format (*.gif)|*.gif|" + "Jpeg Format (*.jpg)|*.jpg|"
        ...
        /// <summary>Handler for the "Save Image As" context menu item.  Copies the current image to the selected Emf format file.</summary>
        /// <remarks>Note that this handler saves as an Emf format only.  The default handler is
        /// <see cref="SaveAs()" />, which allows for Bitmap or EMF formats.</remarks>
        ...
        /// <summary>
        /// Save the current Graph to the specified filename in EMF (vector) format. See <see cref="SaveAsEmf()"/> for public access.
        /// </summary>
        /// <param name="fileName">
        /// The file Name.
        /// </param>
        ...
        internal class ClipboardMetafileHelper
        {
            ...
            // Metafile mf is set to a RequestState that is not valid inside this function. 
            /// <summary>
                ...
                public void ZoomOutAll( GraphPane primaryPane )
                {
                    if ( primaryPane != null && !primaryPane.ZoomStack.IsEmpty )
                    {
                        ...
                        if ( _isSynchronizeXAxes || _isSynchronizeYAxes )
                        {
                            foreach ( GraphPane pane in _masterPane._paneList )
                            {
                                ZoomState RequestState = pane.ZoomStack.PopAll( pane );
                                if ( pane == primaryPane )
        ...
        */
        ...
        /// </summary>
        /// <remarks>
        /// Triggers a <see cref="ZoomEvent"/> for any type of undo (including pan, scroll, zoom, and wheelzoom).  This method will affect all the
