...
namespace SkyDean.FareLiz.WinForm.Components.Controls.Graph
{
	...
	#region Properties
		...
		/// <summary>
		/// Gets or sets the <see cref="Graph.Border"/> data for this
		...
		{
			get { return this._userSymbol; }
			set { this._userSymbol = value; }
		}
					...
					using ( Brush brush = source.Fill.MakeBrush( rect ) )
					{
						...
						for ( int i = 0; i < points.Count; i++ )
						{
									...
									( xIsOrdinal || ( curX >= xMin && curX <= xMax ) ) )
							{
								...
								{
									using ( Brush tBrush = this._fill.MakeBrush( rect, points[i] ) )
									using ( Pen tPen = this._border.GetPen( pane, scaleFactor, points[i] ) )
