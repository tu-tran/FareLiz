...
namespace SkyDean.FareLiz.WinForm.Components.Controls.Graph
{
    ...
    /// <summary>A class representing all the characteristics of the Line segments that make up a curve on the graph.</summary>
    ...
    public class Line : LineBase, ICloneable, ISerializable
    {
        ...
        /// <summary>A simple struct that defines the default property values for the <see cref="Line" /> class.</summary>
        public new struct Default
        {
            // Default Line properties
            /// <summary>
            /// The default color for curves (line segments connecting the points). This is the default value for the <see cref="LineBase.Color" />
            /// property.
            /// </summary>
            public static Color Color = Color.Red;

            /// <summary>The default color for filling in the area under the curve (<see cref="Fill.Color" /> property).</summary>
            public static Color FillColor = Color.Red;

            /// <summary>The default custom brush for filling in the area under the curve (<see cref="Fill.Brush" /> property).</summary>
            public static Brush FillBrush = null;

            /// <summary>The default fill mode for the curve (<see cref="Fill.Type" /> property).</summary>
            public static FillType FillType = FillType.None;

            /// <summary>The default value for the <see cref="Line.IsSmooth" />
            /// property.</summary>
            public static bool IsSmooth = false;

            /// <summary>The default value for the <see cref="Line.SmoothTension" /> property.</summary>
            public static float SmoothTension = 0.5F;

            /// <summary>The default value for the <see cref="Line.IsOptimizedDraw" /> property.</summary>
            public static bool IsOptimizedDraw = false;

            /// <summary>
            /// Default value for the curve type property (<see cref="Line.StepType" />).  This determines if the curve will be drawn by directly connecting the
            /// points from the
            /// <see cref="CurveItem.Points" /> data collection, or if the curve will be a "stair-step" in which the points are connected by a series of horizontal
            /// and vertical lines that represent discrete, staticant values.  Note that the values can be forward oriented <code>ForwardStep</code> (
            /// <see cref="StepType" />) or rearward oriented <code>RearwardStep</code>. That is, the points are defined at the beginning or end of the staticant
            /// value for which they apply, respectively.
            /// </summary>
        ...
        /// <summary>Private field that stores the smoothing flag for this
        /// <see cref="Line" />.  Use the public property <see cref="IsSmooth" /> to access this value.</summary>
        private bool _isSmooth;

        /// <summary>
        /// Private field that stores the smoothing tension for this <see cref="Line" />.  Use the public property
        /// <see cref="SmoothTension" /> to access this value.
        /// </summary>
        /// <value>A floating point value indicating the level of smoothing. 0.0F for no smoothing, 1.0F for lots of smoothing, >1.0 for odd smoothing.</value>
        ...
        /// <summary>
        /// Private field that stores the <see cref="Graph.StepType" /> for this
        /// <see cref="CurveItem" />.  Use the public property <see cref="StepType" /> to access this value.
        /// </summary>
        private StepType _stepType;

        /// <summary>
        /// Private field that stores the <see cref="Graph.Fill" /> data for this
        /// <see cref="Line" />.  Use the public property <see cref="Fill" /> to access this value.
        /// </summary>
        private Fill _fill;

        /// <summary>
        /// Private field that determines if this <see cref="Line" /> will be drawn with optimizations enabled.  Use the public property
        /// <see cref="IsOptimizedDraw" /> to access this value.
        /// </summary>
        ...
        /// will be drawn smooth.  The "smoothness" is controlled by the <see cref="SmoothTension" /> property.
        /// </summary>
        /// <value>true to smooth the line, false to just connect the dots with linear segments</value>
        ...
        /// <summary>
        /// Gets or sets a property that determines the smoothing tension for this <see cref="Line" />.  This property is only used if
        /// <see cref="IsSmooth" /> is true.  A tension value 0.0 will just draw ordinary line segments like an unsmoothed line.  A tension value of 1.0 will be
        /// smooth.  Values greater than 1.0 will generally give odd results.
        /// </summary>
        /// <value>A floating point value indicating the level of smoothing. 0.0F for no smoothing, 1.0F for lots of smoothing, >1.0 for odd smoothing.</value>
        /// <seealso cref="IsSmooth" />
        /// <seealso cref="Default.IsSmooth" />
        ...
        /// <summary>
        /// Determines if the <see cref="CurveItem" /> will be drawn by directly connecting the points from the <see cref="CurveItem.Points" /> data collection,
        /// or if the curve will be a "stair-step" in which the points are connected by a series of horizontal and vertical lines that represent discrete,
        /// constant values.  Note that the values can be forward oriented <c>ForwardStep</c> (<see cref="Graph.StepType" />) or rearward oriented
        /// <c>RearwardStep</c>. That is, the points are defined at the beginning or end of the constant value for which they apply, respectively. The
        /// <see cref="StepType" /> property is ignored for lines that have <see cref="IsSmooth" /> set to true.
        /// </summary>
        /// <value><see cref="Graph.StepType" /> enum value</value>
        ...
        /// <summary>Gets or sets a boolean value that determines if this <see cref="Line" /> will be drawn with optimizations enabled.</summary>
        /// <remarks>
        /// Normally, the optimizations can be used without a problem, especially if the data are sorted.  The optimizations are particularly helpful with very
        /// large datasets. However, if the data are very discontinuous (for example, a curve that doubles back on itself), then the optimizations can cause
        /// drawing artifacts in the form of missing line segments.  The default option for this mode is false, so you must explicitly enable it for each
        /// <see cref="LineItem.Line">LineItem.Line</see>. Also note that, even if the optimizations are enabled explicitly, no actual optimization will be done
        /// for datasets of less than 1000 points.
        /// </remarks>
        public bool IsOptimizedDraw
        {
            get
            {
                return this._isOptimizedDraw;
            }
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="Line"/> class. Default constructor that sets all <see cref="Line"/> properties to default values as defined in the <see cref="Default"/> class.
        /// </summary>
        public Line()
            : this(Color.Empty)
        {
        ...
        public void DrawCurve(Graphics g, GraphPane pane, CurveItem curve, float scaleFactor)
        {
            ...
            int tmpX, tmpY, lastX = int.MaxValue, lastY = int.MaxValue;
                ...
                if (points != null && !this._color.IsEmpty && this.IsVisible)
                {
                    ...
                    for (int i = 0; i < points.Count; i++)
                    {
                        ...
                        else
                        {
                            ...
                            if (isOptDraw && tmpX >= minX && tmpX <= maxX && tmpY >= minY && tmpY <= maxY)
                            {
