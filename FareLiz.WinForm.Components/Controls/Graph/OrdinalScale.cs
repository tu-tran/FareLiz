...
namespace SkyDean.FareLiz.WinForm.Components.Controls.Graph
{
    ...
    /// <summary>
    /// The OrdinalScale class inherits from the <see cref="Scale" /> class, and implements the features specific to <see cref="AxisType.Ordinal" />
    /// .
    /// </summary>
    /// <remarks>
    /// OrdinalScale is an ordinal axis with tic labels generated at integral values.  An ordinal axis means that all data points are evenly spaced at
    /// integral values, and the actual coordinate values for points corresponding to that axis are ignored.  That is, if the X axis is an ordinal type, then
    /// all X values associated with the curves are ignored.
    /// </remarks>
    ...
    internal class OrdinalScale : Scale, ISerializable
    {
        ...
        /// <see cref="AxisType.Ordinal" />.</summary>
        ...
        /// </summary>
        /// <remarks>
        /// This method only applies to <see cref="AxisType.Ordinal"/> type axes, and it is called by the general <see cref="Scale.PickScale"/> method.  The
        /// scale range is chosen based on increments of 1, 2, or 5 (because they are even divisors of 10). Being an ordinal axis type, the
        /// <see cref="Scale.MajorStep"/> value will always be integral.  This method honors the <see cref="Scale.MinAuto"/>, <see cref="Scale.MaxAuto"/>, and
        ...
        /// </param>
        protected OrdinalScale(SerializationInfo info, StreamingContext context)
            : base(info, context)
        {
