...
namespace SkyDean.FareLiz.WinForm.Components.Controls.Graph
{
    ...
    /// <summary>Encapsulates a bar type that displays vertical or horizontal bars</summary>
    /// <remarks>
    /// The orientation of the bars depends on the RequestState of
    /// <see cref="BarSettings.Base" />, and the bars can be stacked or clustered, depending on the RequestState of <see cref="BarSettings.Type" />
    /// </remarks>
    ...
    public class BarItem : CurveItem, ICloneable, ISerializable
    {
        ...
        /// <summary>
        /// Private field that stores a reference to the <see cref="Graph.Bar" />
        /// class defined for this <see cref="BarItem" />.  Use the public property <see cref="Bar" /> to access this value.
        /// </summary>
        protected Bar _bar;
        ...
        /// <summary>Gets a reference to the <see cref="Graph.Bar" /> class defined for this <see cref="BarItem" />.</summary>
        public Bar Bar
        {
            get
            {
        ...
        public override void Draw(Graphics g, GraphPane pane, int pos, float scaleFactor)
        {
            ...
            if (this._isVisible)
            {
                this._bar.DrawBars(g, pane, this, this.BaseAxis(pane), this.ValueAxis(pane), this.GetBarWidth(pane), pos, scaleFactor);
            }
        }
        ...
        public override bool GetCoords(GraphPane pane, int i, out string coords)
        {
            ...
            if (!this._points[i].IsInvalid3D)
            {
                ...
                float pixSide = pixBase - clusterWidth / 2.0F + clusterGap / 2.0F + pane.CurveList.GetBarItemPos(pane, this) * (barWidth + barGap);

                // Draw the bar
                if (baseAxis is XAxis || baseAxis is X2Axis)
                {
                    coords = string.Format("{0:f0},{1:f0},{2:f0},{3:f0}", pixSide, pixLowVal, pixSide + barWidth, pixHiVal);
                }
                else
                {
                    coords = string.Format("{0:f0},{1:f0},{2:f0},{3:f0}", pixLowVal, pixSide, pixHiVal, pixSide + barWidth);
                }
