namespace SkyDean.FareLiz.WinForm.Components.Controls.ComboBox
{
    ...
    /// <summary>
    /// Martin Lottering : 2007-10-27 -------------------------------- This is a usefull control in Filters. Allows you to save space and can
    /// replace a Grouped Box of CheckBoxes. Currently used on the TasksFilter for TaskStatusses, which means the user can select which Statusses to include
    /// in the "Search". This control does not implement a CheckBoxListBox, instead it adds a wrapper for the normal ComboBox and Items. See the
    /// CheckBoxItems property. ---------------- ALSO IMPORTANT: In Data Binding when setting the DataSource. The ValueMember must be a bool type property,
    /// because it will be binded to the Checked property of the displayed CheckBox. Also see the DisplayMemberSingleItem for more information.
    /// ---------------- Extends the CodeProject PopupComboBox "Simple pop-up control" "http://www.codeproject.com/cs/miscctrl/simplepopup.asp" by Lukasz
    /// Swiatkowski.
    /// </summary>
    public class CheckBoxComboBox : PopupComboBox
    {
        ...
        /// <summary>Initializes a new instance of the <see cref="CheckBoxComboBox" /> class.</summary>
        ...
        internal string GetCSVText(bool skipFirstItem)
        {
            ...
            var StartIndex = this.DropDownStyle == ComboBoxStyle.DropDownList && this.DataSource == null && skipFirstItem ? 1 : 0;
            for (var Index = StartIndex; Index <= this._CheckBoxComboBoxListControl.Items.Count - 1; Index++)
            {
                var Item = this._CheckBoxComboBoxListControl.Items[Index];
                if (Item.Checked)
                {
        ...
        /// <summary>
        /// In DataBinding operations, this property will be used as the DisplayMember in the CheckBoxComboBoxListBox. The normal/existing
        /// "DisplayMember" property is used by the TextBox of the ComboBox to display a concatenated Text of the ite'elected. This concatenation and its
        /// formatting however is controlled by the Binded object, since it owns that property.
        /// </summary>
        private string _DisplayMemberSingleItem;

        /// <summary>The _ must add hidden item.</summary>
        ...
        /// <summary>
        /// A direct reference to the Items of CheckBoxComboBoxListControl. You can use it to Get or Set the Checked status of items manually if you
        /// want. But do not manipulate the List itself directly, e.g. Adding and Removing, since the list is synchronised when shown with the ComboBox.Items. So
        /// for changing the list contents, use Items instead.
        /// </summary>
        [Browsable(false)]
        ...
        /// <summary>
        /// In DataBinding operations, this property will be used as the DisplayMember in the CheckBoxComboBoxListBox. The normal/existing
        /// "DisplayMember" property is used by the TextBox of the ComboBox to display a concatenated Text of the items selected. This concatenation however is
        ...
        /// <summary>
        /// Made this property Browsable again, since the Base Popup hides it. This class uses it again. Gets an object representing the collection of
        /// the items contained in this System.Windows.Forms.ComboBox.
        /// </summary>
        ...
        {
            var ListText = this.GetCSVText(true);
        ...
        {
            foreach (var Item in this.CheckBoxItems)
            {
                if (Item.Checked)
                {
                    Item.Checked = false;
        ...
        /// <summary>The _ check box properties.</summary>
