namespace SkyDean.FareLiz.WinForm.Components.Controls.ComboBox
{
    ...
    /// <summary>
    /// CodeProject.com "Simple pop-up control" "http://www.codeproject.com/cs/miscctrl/simplepopup.asp". Represents a Windows combo box control
    /// with a custom popup control attached.
    /// </summary>
    ...
    {
        /// <summary>
        /// The pop-up wrapper for the dropDownControl. Made PROTECTED instead of PRIVATE so descendent classes can set its Resizable property. Note
        /// however the pop-up properties must be set after the dropDownControl is assigned, since this popup wrapper is recreated when the dropDownControl is
        /// assigned.
        /// </summary>
        ...
        protected override void WndProc(ref Message m)
        {
            if (m.Msg == ((int)W32_WM.WM_REFLECT + (int)W32_WM.WM_COMMAND))
            {
                if (NativeMethods.HiWord(m.WParam) == NativeMethods.CBN_DROPDOWN)
                {
                    ...
                    // on the combobox.
                    var TimeSpan = DateTime.Now.Subtract(this.dropDown.LastClosedTimeStamp);
