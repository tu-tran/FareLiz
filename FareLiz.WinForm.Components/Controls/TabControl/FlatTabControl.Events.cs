namespace SkyDean.FareLiz.WinForm.Components.Controls.TabControl
{
    ...
    {
        /// <summary>The mouse drag threshold.</summary>
        private const int MouseDragThreshold = 5;

        /// <summary>The _drag compensate.</summary>
        private int _dragCompensate;

        /// <summary>The _drag offset.</summary>
        private int _dragOffset;

        /// <summary>The _drag start location.</summary>
        private Point _dragStartLocation = Point.Empty;

        /// <summary>The _drag tab rect.</summary>
        private Rectangle _dragTabRect = Rectangle.Empty;

        /// <summary>The _dropping index.</summary>
        private int _droppingIndex = -1;

        /// <summary>The _is dragging.</summary>
        private bool _isDragging;

        /// <summary>The _last active index.</summary>
        private int _lastActiveIndex = -1;

        // Keep track of last active index in order to reduce redundant poainting        

        /// <summary>The _old scroll value.</summary>
        private int _oldScrollValue;

        /// <summary>The tab page closing.</summary>
        [Category("Action")]
        public event TabControlCancelEventHandler TabPageClosing;

        /// <summary>The tab page reloading.</summary>
        [Category("Action")]
        public event TabControlEventHandler TabPageReloading;

        /// <summary>The tab scrolling.</summary>
        [Category("Action")]
        public event ScrollEventHandler TabScrolling;

        /// <summary>The on create control.</summary>
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            if (this.AllowDrop && this.Alignment <= TabAlignment.Bottom && e.Button == MouseButtons.Left)
            {
                if (this.SelectedIndex != -1)
                {
                    ...
                    if (this._isDragging)
                    {
                        ...
                        var dragRight = this._dragOffset > 0; // Mouse move to the right
                        var testPoint = dragRight
                        ...
                        var index = this.GetActiveIndex(testPoint, false);
                        ...
                        if (index != -1)
                        {
                            var canSwitch = (dragRight && index > this.SelectedIndex) || (!dragRight && index < this.SelectedIndex);
                            if (canSwitch)
                            {
                                ...
                                if (testRect.Contains(testPoint))
                                {
                                    ...
                                    var newOffset = this._dragOffset;
                                    this._dragOffset = 0;
                                    var fromTabOriginalRect = this.GetTabRect(this.SelectedIndex);
                                    var offsetDiff = fromTabOriginalRect.X - toRect.X;
                                    ...
                                    this._droppingIndex = -1;
                                    this._dragStartLocation = e.Location;
                                    this._dragCompensate = this._dragOffset;

                                    // So that the next calculation will persist the same drag offset
                                    this.ResumeLayout();
                                }
                            }
            ...
            var needRefresh = this._isDragging;
            if (!needRefresh)
            {
                var index = this.ActiveIndex;
        ...
        /// <summary>The cancel drag.</summary>
        ...
        {
            var activeIdx = (int)this.tabPageContextMenu.Tag;
        ...
        private void mnuCloseAllTabs_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                var totalCount = this.TabPages.Count;
        ...
        private void mnuCloseRightTabs_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                for (var i = activeIdx + 1; i < this.TabPages.Count;)
                {
                    this.CloseTab(i);
                }
            }
