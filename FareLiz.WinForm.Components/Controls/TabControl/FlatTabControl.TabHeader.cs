namespace SkyDean.FareLiz.WinForm.Components.Controls.TabControl
{
    ...
    {
        /// <summary>The _inactive tab diff.</summary>
        private readonly int _inactiveTabDiff = 3;
        ...
        private void DrawTab(Graphics g, TabPage tabPage, int nIndex)
        {
            ...
            {
                using (var tabPageBorderPath = this.GetTabPageBorder(nIndex))
                {
            ...
            // ----------------------------
            // draw tab's icon            
            var hasIcon = tabPage.ImageIndex >= 0 && this.ImageList != null && tabPage.ImageIndex < this.ImageList.Images.Count;
            if (hasIcon)
            {
                ...
                if (tabImage != null)
                {
                    using (tabImage)
                    {
                        var nLeftMargin = 14 + (bSelected ? 2 : 0);
                        const int nRightMargin = 2;
                        var nWidth = tabImage.Width - (bSelected ? 0 : 2);
                        var nHeight = tabImage.Height - (bSelected ? 0 : 2);

                        var rimage = new Rectangle(recBounds.X + nLeftMargin, recBounds.Y + 1, nWidth, nHeight);
        ...
        private Brush GetTabBackgroundBrush(int index)
        {
            ...
            else if (this.HotTrack && index == this.ActiveIndex)
            {
                ...
                var overUpDown = scRect.Contains(mousePos);
                if (!overUpDown)
                {
        ...
        private Rectangle GetCloserRect(int index)
        {
            ...
            var closerSize = rect.Height - 12;
            if (this.Alignment <= TabAlignment.Bottom)
            {
                // Top or Bottom
                var offsetX = this.Padding.X + closerSize;
                if (this.RightToLeftLayout)
                {
            ...
            {
                var offsetY = this.Padding.Y + closerSize;
                if (this.RightToLeftLayout)
                {
                    closerRect = new Rectangle(
            ...
            {
                var diff = this._inactiveTabDiff / 2;
        ...
        private void DrawCloser(int index, Graphics graphics)
        {
            ...
            var closerRect = this.GetCloserRect(index);
            var curMouse = this.PointToClient(MousePosition);
            var mouseOverCloser = closerRect.Contains(curMouse);
            ...
            using (var closerPath = GetCloserPath(closerRect))
            {
                ...
                var closerColor = mouseOverCloser ? Color.White : Color.LightSlateGray;
        ...
        private void DrawRefresher(int index, Graphics graphics)
        {
            ...
            var curMouse = this.PointToClient(MousePosition);
            var mouseOverRefresher = refresherRect.Contains(curMouse);
