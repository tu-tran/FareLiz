namespace SkyDean.FareLiz.WinForm.Components.Controls.Custom
{
    ...
    public partial class FlightItemsPanel : UserControl
    {
        ...
        /// <summary>The _price font.</summary>
        private readonly Font _priceFont;

        /// <summary>The _hover row.</summary>
        private int _hoverRow = -1;

        /// <summary>Initializes a new instance of the <see cref="FlightItemsPanel" /> class.</summary>
        ...
        public int ExpectedHeight
        {
            get
            {
                var labelHeight = this.lblHeader.Height + this.lblHeader.Padding.Top + this.lblHeader.Padding.Bottom;
                var flightHeight = this.gvFlightItems.Rows.GetRowsHeight(DataGridViewElementStates.Visible);
        ...
        internal Size AutoResize(int maxHeight)
        {
            ...
            var targetHeight = this.ExpectedHeight;
            var borderWidth = 2 * SystemInformation.BorderSize.Width;

            var flightWidth = this.gvFlightItems.Columns.GetColumnsWidth(DataGridViewElementStates.Visible) + borderWidth;
            if (targetHeight >= maxHeight)
            {
                flightWidth += SystemInformation.VerticalScrollBarWidth;
            }

            var titleWidth = this.lblHeader.Width + this.lblHeader.Padding.Left + this.lblHeader.Padding.Right;

            var targetWidth = titleWidth > flightWidth ? titleWidth : flightWidth;
        ...
        public void Bind(string header, IList<FlightMonitorItem> data)
        {
            ...
            try
            {
                ...
                var rows = this.GetDataRows(data);
                if (rows != null)
                {
        ...
        {
            var dataCount = data == null ? 0 : data.Count;
            if (dataCount == 0)
            {
                return null;
            }
            ...
            for (var i = 0; i < dataCount; i++)
            {
                ...
                var firstPrice = item.FlightStatus == FlightStatus.New ? flightData.Price : item.OldPrice;
                var roundedFirstPrice = Math.Round(firstPrice);
                ...
                else
                {
                    ...
                    // New price
                    var roundedSecondPrice = Math.Round(flightData.Price);
        ...
        private void gvFlightItems_CellPainting(object sender, DataGridViewCellPaintingEventArgs e)
        {
            ...
            if (e.FormattedValue != null)
            {
                if (cellColumn == this.colFirstPrice || cellColumn == this.colSecondPriceCurrency)
                {
                    ...
                    if (val.Length > 0)
                    {
                        ...
                        var txtX = rect.X + (cellColumn == this.colFirstPrice ? 4 : -2);
                        var txtY = rect.Y + ((rect.Height - txtSize.Height) / 2) + 1;
            ...
            using (var edgePen = (cellColumn == this.colIcon) ? new Pen(ControlPaint.Dark(e.CellStyle.BackColor), 3) : null)
            {
            ...
            if (e.ColumnIndex == 0)
            {
                ...
                if (flightItem != null)
                {
                    var edgeSize = rect.Height / 3;
                    var status = flightItem.FlightStatus;
                    var brush = status == FlightStatus.PriceIncreased
                                    ? this._incPriceBrush
                                    : (status == FlightStatus.PriceDecreased ? this._decPriceBrush : this._newPriceBrush);

                    if (this._hoverRow == e.RowIndex)
                    {
                        var arrowSize = rect.Height / 2;
                        var arrowSpan = (int)(arrowSize * 1.2 / 2);

                        var arrowXLeft = rect.Left - 1;
                        var arrowXMiddle = rect.Left + arrowSize - 1;

                        var arrowYTop = rect.Top + 2;
                        var arrowYMiddle = arrowYTop + arrowSpan;
                        var arrowYBottom = arrowYMiddle + arrowSpan;
        ...
        private void gvFlightItems_CellMouseEnter(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                var data = flightItem.FlightData;
                var row = this.gvFlightItems.Rows[e.RowIndex];
                var canPurchase = data != null && data.CanBePurchased;
                ...
                if (this._hoverRow != e.RowIndex)
                {
                    ...
                    var caption = (canPurchase ? "Double-click to buy this ticket:" : "This ticket cannot be bought at the moment!")
                    ...
                    var rowMiddleLeft = new Point(relRowRect.X, relRowRect.Y + (relRowRect.Height / 2));
