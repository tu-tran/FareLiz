namespace SkyDean.FareLiz.WinForm.Components.Controls.Custom
{
    ...
    public sealed class WebFareBrowserControl : Control, IFareBrowserControl
    {
        ...
        public int TimeoutInSeconds { get { return this.DataHandler == null ? 0 : this.DataHandler.TimeoutInSeconds; } }
        ...
        public TravelRoute LastRetrievedRoute { get { return this._lastRetrievedRoute; } }
        ...
        public DataRequestState RequestState
        {
            ...
            private set { this._requestState = value; }
        }

        public event JourneyBrowserDelegate GetJourneyCompleted;

        private WebFareBrowserControl() : this(null) { }
        ...
        private void DoRequestData(FlightFareRequest request)
        {
            ...
            try
            {
                ...
                this.lvFlightData.SafeInvoke(new Action(() =>
                    {
                ...
                var workResult = BackgroundThread.DoWork(() =>
                    {
                        this.lvFlightData.SetWatermark("Data request was started on " + DateTime.Now + Environment.NewLine + "Please wait while the application retrieves fare data...");
                    ...
                    }, this.TimeoutInSeconds, reqName, AppContext.Logger);
        ...
        private void BindData(TravelRoute route)
        {
            if (route != null && route.Journeys.Count > 0 && route.Journeys[0].Data.Count > 0)
            {
                ...
                this.lvFlightData.SetWatermark("There is no data for selected journey. Probably there is no flight on that date or there is no flight between two destinations!");
